...
describe("Stargate fees", function () {
  it("Contract Extracted fees = Typescript calculated fees", async function () {
    ...
    const FeeCalcFactory = await hre.ethers.getContractFactory(
    ...
    for (let i = 0; i < 10; ++i) {
      ...
      for (let j = 0; j < 10; ++j) {
        const amountSD = getRandom(
          rnd,
          Math.pow(currentBalance, 0.25),
          currentBalance
        );

        const feesContract = await FeeCalcContract.getFees(
          bridgeState,
          whitelisted,
          amountSD
        );
        const feesTypescript = getStarGateFeesV04(
          bridgeState,
          whitelisted,
          amountSD
        );
        ...
        expect(feesTypescript.protocolFee.eq(feesContract.protocolFee)).equal(
          true
        );
      }
    }
  ...
  it("Ethereum USDC - Polygon USDC fees check", async () => {
    ...
    for (let n = 1; n <= 10; ++n) {
      ...
      expect(bridgeFees.eqReward.toString()).equal(
        calcFees.eqReward.toString()
      );
      expect(bridgeFees.lpFee.toString()).equal(calcFees.lpFee.toString());
      expect(bridgeFees.protocolFee.toString()).equal(
        calcFees.protocolFee.toString()
      );
    }
  });
});

async function getStargateBridgeState_USDC_USDC(
  srcChain: ChainId,
  dstChain: ChainId,
  from: string
) {
...
async function getStargateBridgeState(
  ...
  const feeLibrary = await new ethers.Contract(
    feeLibraryAddress,
    StargateFeeLibraryABI,
    ethers.getDefaultProvider()
  );
  const lpStakingAddress = await feeLibrary.lpStaking();
  const lpStaking = await new ethers.Contract(
    lpStakingAddress,
