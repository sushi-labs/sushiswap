...
import { Native, Token, USDC, WNATIVE } from '@sushiswap/currency'
...
if (process.env.INFURA_API_KEY) {
  describe('Celo test', () => {
    it.only('Celo test', async () => {
      ...
      for (let i = 0; i < 10; ++i) {
        // try to find a route
        route = Router.findBestRoute(dataFetcher, fromToken, getBigNumber(10 * 1e18), toToken, 50 * 1e9)
      ...
      if (route) {
        ...
        const rpParams = Router.routeProcessorParams(
          ...
          fromToken,
          toToken,
          to, //WNATIVE[chainId].address, // to
          RouteProcessorAddr
        )

        console.log(rpParams)

        //const RouteProcessor = new Contract(RouteProcessorAddr, RPData.abi, provider.getSigner())
        ...
        // const p = await provider.perform('call', {
        //   transaction: {
