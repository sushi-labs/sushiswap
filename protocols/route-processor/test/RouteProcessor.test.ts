...
async function BentoMakeTokenStrategyPercentage(
  ...
  const bentoOwner = await ethers.getSigner(
...
async function testRouteProcessor(
  chainId: ChainId,
  amountIn: number,
  toToken: Token,
  swaps = 1
  ...
  const RouteProcessor: RouteProcessor__factory =
    await ethers.getContractFactory("RouteProcessor");
  ...
  console.log(
    `3. Deposit user's ${amountIn} ${baseWrappedToken.symbol} to ${baseWrappedToken.symbol}`
  );
  ...
  console.log(
    `4. Approve user's ${amountIn} ${baseWrappedToken.symbol} to the route processor ...`
  );
  ...
  Object.keys(swapper.poolsNumber).forEach((provider) => {
    console.log(
      `    ${provider}: ${swapper.poolsNumber[provider]} pools were found`
    );
  });
  ...
  if (route.status == RouteStatus.NoWay) {
    ...
    console.log("    Route Status: ", route.status);
    console.log(
      `    Input: ${Math.round(route.amountIn / Math.pow(10, 18))} ${
        route.fromToken.name
      }`
    );
    route.legs.forEach((l, i) => {
      console.log(
        `    ${i + 1}. ${l.tokenFrom.name} ${Math.round(
          l.absolutePortion * 100
        )}%` +
          ` -> [${swapper.getPoolsProviderName(l.poolAddress)}] -> ${
            l.tokenTo.name
          }`
      );
    ...
    const output =
      Math.round(
        (parseInt(route.amountOutBN.toString()) /
  ...
  for (let i = 0; i < swaps; ++i) {
    ...
    const amountOutMin =
      i > 0
        ? getBigNumber(0)
        : route.amountOutBN
            .mul(getBigNumber((1 - 0.005) * 1_000_000))
            .div(1_000_000);
...
describe("RouteProcessor", async function () {
  ...
  it.skip("Polygon max output limitation", async function () {
    ...
    if (forking_url !== undefined && forking_url.search("polygon") >= 0) {
      ...
      await BentoMakeTokenStrategyPercentage(
        ChainId.POLYGON,
        WBTC_Token.address,
        60
      );
