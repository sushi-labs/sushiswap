...
function sortTokens(tokens: Token[]): Token[] {
  const t1: [Token, BigNumber][] = tokens.map((t) => [
    t,
    BigNumber.from(t.address),
  ]);
  t1.sort(([, a0], [, a1]) => {
...
export class TridentProvider extends LiquidityProvider {
  ...
  async _getTokenPaiPoolCodes(
    ...
    for (let k = 0; k < paiPoolCodes.length; ++k) {
      ...
      const [res0, res1]: [BigNumber, BigNumber] = await this.limited.call(() =>
        poolContract.getReserves()
      );
      const fee: BigNumber = await this.limited.call(() =>
      ...
      const poolCode = new BentoConstantProductPoolCode(
        pool,
  ...
  async _getAllBridges(
    ...
    const promises = Array.from(tokenBentoMap.values()).map(async (t) => {
      ...
      const max: BigNumber = await this.limited.call(() =>
        TokenContract.balanceOf(BentoBox[this.chainId])
      );
      ...
      const br = new BentoBridgePoolCode(
        pool,
        this.getPoolProviderName(),
        BentoBox[this.chainId]
      );
