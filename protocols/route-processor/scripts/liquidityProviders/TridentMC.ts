...
export class TridentProviderMC extends LiquidityProviderMC {
  ...
  async getAllTridentPools(tokensSorted: Token[]): Promise<PoolCode[]> {
    ...
    const swapFeePromise = this.multiCallProvider.multiContractCall(
      poolAddr,
      swapFeeABI,
      "swapFee",
      []
    );
    const [poolRes0, poolFee0] = await Promise.all([
      poolReservePromise,
      swapFeePromise,
    ]);
    ...
    poolAddr.forEach((addr, i) => {
      ...
      if (res !== undefined && fee !== undefined) {
        ...
        poolCodes.push(
          new BentoConstantProductPoolCode(pool, this.getPoolProviderName())
        );
      }
    });
  ...
  async getAllBridges(tokensSorted: Token[]): Promise<PoolCode[]> {
    ...
    const [totals0, balances0] = await Promise.all([
      totalsPromise,
      balancesPromise,
    ]);
  ...
  async updatePoolsData() {
    ...
    this.poolCodes.forEach((pc) =>
      pc instanceof BentoConstantProductPoolCode
        ? pools.push(pc)
        : bridges.push(pc)
    );
    ...
    const [reserves0, totals0, balances0] = await Promise.all([
      reservesPromise,
      totalsPromise,
      balancesPromise,
    ]);
