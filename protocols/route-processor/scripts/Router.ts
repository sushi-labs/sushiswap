...
export class Router {
  ...
  _checkRouteUpdate() {
    ...
    if (this.dataFetcherPreviousState != currentDataFetcherStateId) {
      ...
      const route = findMultiRouteExactIn(
        ...
        this.amountIn,
        this.dataFetcher
          .getCurrentPoolCodeList(this.providers)
          .map((pc) => pc.pool),
        networks,
        this.gasPrice
      );
  ...
  getRPParams(
    ...
    const tokenIn =
      fromToken instanceof Token
        ? fromToken.address
        : WNATIVE_ADDRESS[fromToken.chainId];
    const tokenOut =
  ...
  routeToHumanString(
    ...
    route.legs.forEach((l, i) => {
      res +=
        shiftSub +
        shiftSub +
        `${i + 1}. ${l.tokenFrom.symbol} ${Math.round(
  ...
  getCurrentRouteHumanString(
    shiftPrimary = "",
    shiftSub = "    "
  ): string | undefined {
    if (this.currentBestRoute !== undefined) {
      return this.routeToHumanString(
        this.currentBestRoute,
        this.fromToken,
        this.toToken,
        shiftPrimary,
