...
export class TinesToRouteProcessor {
  ...
  codeDistributeInitial(route: MultiRoute): [string, Map<string, BigNumber>] {
    ...
    console.assert(
      code.length == (2 + legsAddr.length * 52) * 2,
      "codeDistributeInitial unexpected code length"
    );
  ...
  codeDistributeTokenShares(token: RToken, route: MultiRoute): string {
    ...
    if (RPStartPointsNum > 1) {
      throw new Error(
        "More than one RouteProcessor for a token is not supported"
      );
    }
    ...
    const code = hex.toString();
    console.assert(
      code.length == (22 + legs.length * 22) * 2,
