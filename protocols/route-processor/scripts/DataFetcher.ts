...
import {
  LiquidityProviderMC,
  LiquidityProviders,
} from "./liquidityProviders/LiquidityProviderMC";
...
export class DataFetcher {
  ...
  fetchPoolsForToken(t0: Type, t1: Type) {
    ...
    this.providers.forEach((p) =>
      p.fetchPoolsForToken(t0 as Token, t1 as Token)
    );
  }
  ...
  getCurrentPoolCodeMap(
    ...
    this.providers.forEach((p) => {
      ...
      if (
        p.getCurrentPoolStateId() !== this.lastProviderStates.get(p.getType())
      ) {
        ...
        poolCodes.forEach((pc) =>
          (pcMap as Map<string, PoolCode>).set(pc.pool.address, pc)
        );
      }
      const pcMap = this.poolCodes.get(p.getType());
      if (pcMap)
        Array.from(pcMap.entries()).forEach(([addr, pc]) =>
          result.set(addr, pc)
        );
    });
