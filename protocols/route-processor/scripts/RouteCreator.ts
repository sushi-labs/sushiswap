...
import {
  ...
  getBentoChainId,
  TridentProvider,
} from "./liquidityProviders/Trident";
...
export class RouteCreator {
  ...
  stopRouting() {
    if (this.routingState?.timer !== undefined)
      clearTimeout(this.routingState?.timer);
    ...
    if (this.providers.some((p) => p.poolListWereUpdated())) {
      ...
      if (route.status != RouteStatus.NoWay) {
        if (
          ...
          route.totalAmountOut > this.currentBestRoute.totalAmountOut
        ) {
  ...
  getRouteProcessorCodeForBestRoute(
  ...
  routeToString(
    ...
    res +=
      shiftPrimary +
      `Input: ${route.amountIn / Math.pow(10, fromToken.decimals)} ${
        fromToken.symbol
      }\n`;
    route.legs.forEach((l, i) => {
      res +=
        ...
        ` -> [${this.poolCodes.get(l.poolAddress)?.poolName}] -> ${
          l.tokenTo.name
        }\n`;
      //console.log(l.poolAddress, l.assumedAmountIn, l.assumedAmountOut)
    });
    const output =
      parseInt(route.amountOutBN.toString()) / Math.pow(10, toToken.decimals);
