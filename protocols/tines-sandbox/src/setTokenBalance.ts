...
const TokenProxyMap: Record<string, string> = {
  ...
  '0x056fd409e1d7a124bd7017459dfea2f387b6d5cd': '0xc42B14e49744538e3C239f8ae48A1Eaaf35e68a0', // Ethereum GUSD
}

const cache: Record<string, number> = {}

export async function setTokenBalance(token: string, user: string, balance: bigint): Promise<boolean> {
  const setStorage = async (realContract: string, slotNumber: number, value0: NumberLike, value1: NumberLike) => {
    // Solidity mapping
  ...
  for (let i = 0; i < 200; ++i) {
    const [previousValue0, previousValue1] = await setStorage(realContract, i, balance, balance)
