                    ...
                    case 1:
                        ...
                        try {
                            for (_b = __values(this.cliqueActiveSigners()), _c = _b.next(); !_c.done; _c = _b.next()) {
                                signer = _c.value;
                                debug("Clique signer [".concat(i, "]: ").concat(signer));
                                i++;
                            }
                        }
                                ...
                                switch (_e.label) {
                                    case 0:
                                        ...
                                        if (numBeneficiaryVotesAUTH >= limit_1) {
                                            ...
                                            debug("[Block ".concat(header.number, "] Clique majority consensus (AUTH ").concat(beneficiary_1, ")"));
                                        }
                                        // DROP vote
                                        votes = this_1._cliqueLatestVotes.filter(function (vote) {
                                            return (vote[0].gte(lastEpochBlockNumber) &&
                                        ...
                                        if (numBeneficiaryVotesDROP >= limit_1) {
                                            ...
                                            debug("[Block ".concat(header.number, "] Clique majority consensus (DROP ").concat(beneficiary_1, ")"));
                                        }
                                        if (round === 1) {
                                            ...
                                            debug("[Block ".concat(header.number, "] New clique vote: ").concat(signer_1, " -> ").concat(beneficiary_1, " ").concat(nonce.equals(clique_1.CLIQUE_NONCE_AUTH) ? 'AUTH' : 'DROP'));
                                        }
                                        if (!consensus) return [3 /*break*/, 2];
                                        if (round === 1) {
                                            debug("[Block ".concat(header.number, "] Clique majority consensus on existing votes -> update signer states"));
                                        }
                                        else {
                                            debug("[Block ".concat(header.number, "] Clique majority consensus on new vote -> update signer states"));
                                        }
                                                ...
                                                try {
                                                    for (_a = __values(checkpointSigners.entries()), _b = _a.next(); !_b.done; _b = _a.next()) {
                                                        ...
                                                        if (!activeSigners[i] || !activeSigners[i].equals(cSigner)) {
                                                            throw new Error("checkpoint signer not found in active signers list at index ".concat(i, ": ").concat(cSigner));
                                                        }
                                                    }
                                                }
