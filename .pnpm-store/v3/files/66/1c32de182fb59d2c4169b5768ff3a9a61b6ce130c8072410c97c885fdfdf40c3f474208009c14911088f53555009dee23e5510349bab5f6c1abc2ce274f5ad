...
var Opcode = /** @class */ (function () {
    function Opcode(_a) {
        var code = _a.code, name = _a.name, fullName = _a.fullName, fee = _a.fee, isAsync = _a.isAsync, dynamicGas = _a.dynamicGas;
        this.code = code;
        this.name = name;
...
function getOpcodesForHF(common, customOpcodes) {
    ...
    var handlersCopy = new Map(functions_1.handlers);
    var dynamicGasHandlersCopy = new Map(gas_1.dynamicGasHandlers);
    ...
    for (var key in opcodeBuilder) {
        ...
        if (baseFee === undefined) {
            throw new Error("base fee not defined for: ".concat(opcodeBuilder[key].name));
        }
        opcodeBuilder[key].fee = common.param('gasPrices', opcodeBuilder[key].name.toLowerCase());
    }
    if (customOpcodes) {
        try {
            for (var customOpcodes_1 = __values(customOpcodes), customOpcodes_1_1 = customOpcodes_1.next(); !customOpcodes_1_1.done; customOpcodes_1_1 = customOpcodes_1.next()) {
                var _code = customOpcodes_1_1.value;
                var code = _code;
                ...
                if (code.opcodeName === undefined || code.baseFee === undefined) {
                    throw new Error("Custom opcode ".concat(code.opcode, " does not have the required values: opcodeName and baseFee are required"));
                }
                var entry = (_c = {},
                    _c[code.opcode] = {
                    ...
                    _c);
                opcodeBuilder = __assign(__assign({}, opcodeBuilder), entry);
                if (code.gasFunction) {
            ...
            try {
                if (customOpcodes_1_1 && !customOpcodes_1_1.done && (_b = customOpcodes_1.return)) _b.call(customOpcodes_1);
            }
