...
import {
  CHAINLINK_PRICE_FEED_MAP,
  CHAINLINK_SUPPORTED_CHAIN_IDS,
} from "../src";
describe("price feed mappings", () => {
  it("validates", () => {
    for (const chainId of CHAINLINK_SUPPORTED_CHAIN_IDS) {
      for (const [key, priceFeed] of Object.entries(
        ...
        if (priceFeed.decimals !== priceFeed.toDecimals)
          console.log(chainId, key, priceFeed);
