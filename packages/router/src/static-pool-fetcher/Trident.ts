...
import { constantProductPoolFactoryAbi, constantProductPoolFactoryAddress, ConstantProductPoolFactoryChainId, stablePoolFactoryAbi, stablePoolFactoryAddress, StablePoolFactoryChainId } from '@sushiswap/trident'
...
export class TridentStaticPoolFetcher {
  ...
  private static async getPools(client: PublicClient,
    chainId: ChainId,
    t1: Token,
    t2: Token,
    type: 'STABLE_POOL' | 'CONSTANT_PRODUCT_POOL') {
        ...
        const factoryAbi = type === 'STABLE_POOL' ? stablePoolFactoryAbi[chainId as StablePoolFactoryChainId]
