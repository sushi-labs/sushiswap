...
import { useEffectDebugger, useIsMounted } from '@sushiswap/hooks'
...
export const GenericTable = <T extends { id: string }>({
  ...
  return useMemo(
              ...
              {!loading && table.getRowModel().rows.length === 0 && (
                <Table.tr className="!h-[260px]">
                  <Table.td colSpan={table.getAllColumns().length} className="!h-[260px] pointer-events-none">
                    <Typography variant="xs" className="text-slate-400 w-full text-center">
                      {placeholder}
                    </Typography>
                  </Table.td>
                </Table.tr>
    ...
    [HoverElement, isMounted, linkFormatter, loading, loadingOverlay, pageSize, placeholder, showOverlay, table]
  )
}
