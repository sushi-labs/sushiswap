...
const DialogRoot: FC<DialogRootProps> = ({ open, onClose, children, afterLeave, maxWidth = "md", variant = 'transparent', ...rest }) => {
  const { unmount } = rest
              ...
              <div className="flex items-end justify-center min-h-full text-center sm:items-center">
                ...
                >
                  <HeadlessDialog.Panel
                      className={`w-full h-full ${TailwindMapper[maxWidth]} px-1`}>
                    {children}
                  </HeadlessDialog.Panel>
                </Transition.Child>
              </div>
