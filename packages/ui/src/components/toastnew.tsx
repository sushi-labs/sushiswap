...
>(({ className, ...props }, ref) => (
	<ToastPrimitives.Close
		ref={ref}
		className={classNames(
			"hover:bg-secondary absolute flex items-center justify-center right-0 top-0 bottom-0 rounded-md p-2 text-foreground/50 opacity-0 transition-opacity hover:text-foreground focus:opacity-100 focus:outline-none focus:ring-2 group-hover:opacity-100 group-[.destructive]:text-red-300 group-[.destructive]:hover:text-red-50 group-[.destructive]:focus:ring-red-400 group-[.destructive]:focus:ring-offset-red-600",
			className,
		)}
		toast-close=""
		{...props}
...
const ToastVisual = React.forwardRef<HTMLDivElement, ToastVisual>(
	({ className, ...props }, ref) => (
		<div
			ref={ref}
			className={classNames(
				"ml-[-16px] mr-[-16px] mt-[-16px] w-[calc(100%+32px)] h-[128px] max-h-[128px]",
				className,
			)}
			{...props}
		/>
...
>(({ className, ...props }, ref) => (
	<ToastPrimitives.Description
		ref={ref}
		className={classNames("relative text-sm opacity-90 p-2 pr-10 truncate", className)}
		{...props}
	/>
));
