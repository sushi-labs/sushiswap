...
export class CurveMultitokenPool extends RPool {
  ...
  override updateReserves(_res0: bigint, _res1: bigint) {
    console.error('Not expected to be used')
...
class CurveMultitokenCore {
  ...
  A: number
  reserves: bigint[]
  currentFlow: number[][][] // [from index][to index][0/1/gas], from < to
  ...
  computeY2(xIndex: number, x: bigint, yIndex: number): bigint {
    const D = this.computeLiquidity()
    let c = D
    let S_ = ZERO
    for (let i = 0; i < this.tokens.length; ++i) {
      let _x = ZERO
      if (i == xIndex) _x = x
      else if (i != yIndex) _x = this.currentReserves[i] as bigint
      else continue
      S_ = S_ + _x
      c = (c * D) / _x / this.n
    }
  ...
  setCurrentFlow(from: number, to: number, flow0: number, flow1: number, gas: number) {
    this.currentFlow[from][to] = [flow0, flow1, gas]
  }
}
