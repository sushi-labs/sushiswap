...
export class ParallelCPRPool extends RPool {
  ...
  constructor(
    ...
    super(
      ...
      getBigNumber(
        pools.reduce((a, b) => a + b.reserve1Number / (1 - b.fee), 0)
      )
    );
  ...
  calcNextJumpforPool(
    ...
    const combinedYNew =
      Math.sqrt(poolPrice / prevJump.price) * prevJump.combinedLiquidityY;
    ...
    const inputFirst =
      (prevJump.combinedLiquidityY * outputFirst) /
      prevJump.price /
      combinedYNew;
    ...
    const outputSecond =
      (combinedYNew * inputSecond) / (combinedYNew / poolPrice + inputSecond) +
      swapCost;
    ASSERT(() => {
      const outputSecond2 =
        ((combinedYNew + y) * inputSecond) /
        ((combinedYNew + y) / poolPrice + inputSecond);
      return Math.abs(outputSecond / outputSecond2 - 1) < 1e-12;
    }, "Internal Error 62");
    const combinedYFinal = combinedYNew + y - outputSecond;
    const priceFinal =
      poolPrice * Math.pow(combinedYFinal / (combinedYNew + y), 2);
    return {
      poolIndex,
  ...
  calcOutByIn(
    ...
    const addOutput =
      (jump.combinedLiquidityY * addInput) /
      (jump.combinedLiquidityY / jump.price + addInput);
  ...
  calcInByOut(
    ...
    let addInput =
      ((jump.combinedLiquidityY / jump.price) * addOutput) /
      (jump.combinedLiquidityY - addOutput);
