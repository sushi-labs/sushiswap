...
class Pool {
  ...
  input(): number {
    return this.direction
      ? this.edge.amountInPrevious
      : this.edge.amountOutPrevious;
  }
}
...
export class Redistributor {
  ...
  constructor(_nodesTopologySorted: Vertice[]) {
    ...
    _nodesTopologySorted.forEach((n, i) => {
      ...
      const nodes = edges.map(
        (e) => this.tokenIndex.get(n.getNeibour(e) as Vertice) as number
      );
  ...
  calcInputForPrice(
    ...
    return revertPositive(
      (x) => this.calcPrice(from, to, paths, x),
      price,
      amountIn
    );
  }
