  ...
  calcOutByIn(
    ...
    while (input > 0) {
      ...
      if (startFlag) {
        ...
        const nextTickPriceBN = getSqrtRatioAtTick(
          this.ticks[nextTickToCross].index
        );
        nextTickPrice = parseInt(nextTickPriceBN.toString()) / two96;
        priceDiff =
          parseInt(currentPriceBN.sub(nextTickPriceBN).toString()) / two96;
        startFlag = false;
      } else {
        nextTickPrice = Math.sqrt(
          Math.pow(1.0001, this.ticks[nextTickToCross].index)
        );
      ...
      if (direction) {
        ...
        if (input <= maxDx) {
          ...
          input -= maxDx;
          if (
            (this.ticks[nextTickToCross].index / this.tickSpacing) % 2 ===
            0
          ) {
            currentLiquidityBN = currentLiquidityBN.sub(
              this.ticks[nextTickToCross].DLiquidity
            );
          } else {
            currentLiquidityBN = currentLiquidityBN.add(
              this.ticks[nextTickToCross].DLiquidity
            );
          }
          nextTickToCross--;
