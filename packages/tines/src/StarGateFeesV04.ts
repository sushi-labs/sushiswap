...
function getEqReward(state: BridgeState, amountSD: BigNumber): BigNumber {
  ...
  if (
    state.currentAssetSD.mul(100).div(state.lpAsset).lt(75) &&
    amountSD.mul(100).gt(poolDeficit.mul(2))
  ) {
...
function getEquilibriumFee(
  state: BridgeState,
  amountSD: BigNumber
): { eqFee: BigNumber; protocolSubsidy: BigNumber } {
  ...
  if (afterBalance.gte(safeZoneMax)) {
    ...
    // safe zone
    const proxyBeforeBalance = beforeBalance.lt(safeZoneMax)
      ? beforeBalance
      : safeZoneMax;
...
function getProtocolAndLpFee(
  ...
  const isAboveIdeal = state.currentBalance
    .sub(amountSD)
    .gt(state.idealBalance.mul(SIXTY_PERCENT).div(DENOMINATOR));
  const currentAssetNumerated = state.currentAssetSD
    .mul(DENOMINATOR)
    .div(state.lpAsset);
