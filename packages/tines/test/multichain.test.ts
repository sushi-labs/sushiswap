...
it.skip('two chains', () => {
  ...
  const { pools, networks } = createMultipleNetworks(rnd, [
    {
      tokenNumber: 5,
      density: 0.4,
      gasPrice: 100e9,
  ...
  const gasPrice1 = getBasePrice(network[1], baseToken1)
  ...
  findMultiRouteExactIn(fromToken, toToken, amountIn, pools)
})
