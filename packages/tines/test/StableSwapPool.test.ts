...
function checkCurveInvariant(
  ...
  const x = (direction ? pool.reserve0 : pool.reserve1).add(
    getBigNumber(amountInWithoutFee)
  );
  const y = (direction ? pool.reserve1 : pool.reserve0).sub(
    getBigNumber(amountOut)
  );
...
describe("StableSwap test", () => {
  ...
  it("special case", () => {
    const pool = createPool(
      BigNumber.from(5028472782),
      BigNumber.from(5028350937)
    );
