...
function exportNetwork(
  ...
  route.legs.forEach((l) =>
    usedPools.set(
      l.poolAddress,
      l.tokenFrom === allPools.get(l.poolAddress)?.token0
    )
  );
  ...
  const nodes = `var nodes = new vis.DataSet([
    ${network.tokens
      .map((t) => `{ id: ${t.name}, label: "${nodeLabel(t)}"}`)
      .join(",\n\t\t")}
  ]);\n`;
  const edges = `var edges = new vis.DataSet([
    ${network.pools
      .map(
        (p) => `{ from: ${p.token0.name}, to: ${p.token1.name}${edgeStyle(p)}}`
      )
      .join(",\n\t\t")}
  ]);\n`;
  const data = `var data = {
...
function exportPrices(network: Network, baseTokenIndex: number) {
  ...
  function nodeLabel(t: TToken) {
    const info = `${t.name}:${
      (tokenPriceMap.get(t.name) / atomPrice(t)) * atomPrice(baseToken)
    }`;
...
function printRoute(route: MultiRoute, network: Network) {
  ...
  route.legs.forEach((l, i) => {
    ...
    const diff = numberPrecision(
      100 * ((out * price_out) / inp / price_in - 1)
    );
    info +=
      `${i} ${numberPrecision(l.absolutePortion)} ${l.tokenFrom.name}->${
        l.tokenTo.name
      }` +
...
it("Token price calculation is correct", () => {
  ...
  g.vertices.forEach((v) => {
    ...
    if (v.price !== 0) {
      expectCloseValues(
        v.price,
        atomPrice(v.token) / atomPrice(baseToken),
...
it(`Multirouter for ${network.tokens.length} tokens and ${network.pools.length} pools (200 times)`, () => {
  for (let i = 0; i < 200; ++i) {
    ...
    if (route.priceImpact !== undefined && route.priceImpact < 0.1) {
      ...
      const routeOut = findMultiRouteExactOut(
        t0,
        t1,
        route.amountOut,
        network.pools,
  ...
  for (let i = 0; i < 100; ++i) {
    ...
    if (route.status !== RouteStatus.NoWay) {
      ...
      if (routeOut.status !== RouteStatus.NoWay) {
        const route3 = findSingleRouteExactIn(
          t0,
          t1,
          routeOut.amountIn,
          network.pools,
