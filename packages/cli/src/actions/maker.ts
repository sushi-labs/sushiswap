...
import {
  deprecated_LiquidityPositionsQuery,
  getBuiltGraphSDK,
...
export async function maker(args: Arguments) {
    ...
    MAKER_SUPPORTED_CHAIN_NAMES.map((chainName) => {
      ...
      return sdk
        ...
        .then(({ liquidityPositions }) => {
          if (
            !Array.isArray(liquidityPositions) ||
            !liquidityPositions.length
          ) {
            throbber.warn(
              `We were unable to find any liquidity positions for ${chainName}:${MAKER_ADDRESS[chainId]}`
            );
          }
          throbber.succeed(
  ...
  throbber.stopAndPersist({
    ...
    text: `Found ${results.reduce(
      ...
      0
    )} liquidity positions for Sushi Maker across ${MAKER_SUPPORTED_CHAIN_NAMES.join(
      ", "
    )} chains`,
  });
  ...
  console.log(
    `Total value: ` + chalk.green(`${numeral(totalValue).format("$0.00a")}`)
  );
}
