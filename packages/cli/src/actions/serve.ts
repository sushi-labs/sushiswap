...
export async function serve({ network, verbose }: Arguments) {
  ...
  const burnPairs = liquidityPositions
    .map((burnPair) => ({
      ...burnPair,
      valueUSD:
        (burnPair.liquidityTokenBalance / burnPair.pair.totalSupply) *
        burnPair.pair.reserveUSD,
    }))
  ...
  for (const burnPair of burnPairs) {
    console.log(
      `${chalk.blue(
        `🔥 ${burnPair.pair.token0.symbol}/${burnPair.pair.token1.symbol}`
      )} ${chalk.green(formatUSD(burnPair.valueUSD))}`
    );
  }
