...
export async function chef(args: Arguments) {
  ...
  const { MASTERCHEF_V1_pools, MASTERCHEF_V2_pools } =
    await sdk.MasterChefPools();
  ...
  const digestPools = (
    pools: typeof MASTERCHEF_V1_pools,
    ...
    pools
      ...
      .map((pool) => {
        ...
        const tvl =
          ((pair?.reserveUSD / pair?.totalSupply) * pool.slpBalance) / 1e18;
        const sushiPerSecond =
          ((pool.allocPoint / pool.masterChef.totalAllocPoint) *
            sushiPerBlock) /
          secondsPerBlock;
        const apr =
          (getUnixTime(addYears(0, 1)) * sushiPerSecond * sushiPriceUSD) / tvl;
        return {
          Index: pool.id,
  ...
  if (digestedV1.length > 0) {
    ...
    console.log(
      chalk.blue(
        `Total Alloc Points: ${MASTERCHEF_V1_pools[0].masterChef.totalAllocPoint}`
      )
    );
  ...
  const digestedV2 = digestPools(
    MASTERCHEF_V2_pools,
    (MASTERCHEF_V2_pools[0].masterChef.totalAllocPoint /
      MASTERCHEF_V1_pools[0].masterChef.totalAllocPoint) *
      100
  );
  if (digestedV2.length > 0) {
    const table = new Table({
      head: Object.keys(digestedV2[0]),
      style: { compact: true },
    });
    ...
    console.log(
      chalk.blue(
        `Total Alloc Points: ${MASTERCHEF_V2_pools[0].masterChef.totalAllocPoint}`
      )
    );
    console.log(table.toString());
  }
