...
import {
  FURO_SUPPORTED_CHAIN_NAMES,
  REVENUES_SUPPORTED_CHAIN_NAMES,
} from "../config";
...
export async function furo(args: Arguments) {
  ...
  await fetch(
    "https://coins.llama.fi/prices/current/" +
      tokensDefillamaPriceQuery.join(",")
  ).then((data) =>
    data.json().then((data) => {
  ...
  const tokensTVL = tokens
    .map((token) => {
      const tokenAmount =
        (Number(token.liquidityShares) / 10 ** Number(token.decimals)) *
        rebases[token.id];
  ...
  table.push([
    ...
    chalk.green("-"),
    chalk.green(
      totalTvl.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") + " $"
    ),
  ]);
  for (const token of tokensTVL) {
    table.push([
      token.chain,
      token.symbol,
      token.tvl.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") + " $",
    ]);
  }
