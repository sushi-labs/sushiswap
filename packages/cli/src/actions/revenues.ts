...
export async function revenues(args: Arguments) {
  ...
  if (days < 1 || days > 30) {
    console.log(
      chalk.red("Please provide a valid amount of day between 1 and 30.")
    );
    return;
  }
  if (args.network) {
      ...
      console.log(
        chalk.red(
          "Please provide a valid network: " +
            REVENUES_SUPPORTED_CHAIN_NAMES.join(", ") +
            "."
        )
      );
      return;
    ...
    console.log(
      chalk.green(
        "Querying revenues for all networks during the last " +
          days +
          " day(s)..."
      )
    );
  ...
  const sushiPriceUSD = await (async function () {
    {
      const prices = await fetch("https://token-price.sushi.com/v0/1").then(
        (data) => data.json()
      );
  ...
  const revenues = await processRevenues(
    lastBlocks,
    pastBlocks,
    sushiPriceUSD,
    days
  ...
  for (const revenue of orderedRevenues) {
    ...
    const benefitsString = chalk.bold(
      benefits.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") + " $"
    );
    table.push([
      ...
      "",
      chalk.bold(
        revenue[1].volume.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") +
          " $"
      ),
      chalk.bold(
    ...
    //pairs details
    const topPairs = [
      ...revenue[1].pairs.slice(0, 5),
      ...revenue[1].pairs.slice(-5).reverse(),
    ];
    topPairs.map((pair, i) => {
      const pairBenefits = pair.revenue - pair.spent;
      const pairBenefitsString =
        pairBenefits.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") + " $";
      table.push([
        ...
        pairBenefits > 0
          ? chalk.green(pairBenefitsString)
          : chalk.red(pairBenefitsString),
      ]);
    });
  }
...
async function processRevenues(
  ...
  await Promise.all(
    lastBlocks.map(async (block) => {
      const lastPairs = await getAllPairsWithFarms(
        [block.chainId],
        Number(block.number)
      );
        ...
        if (pair.farm) {
          const sushiIncentive = pair.farm.incentives.find((incentive) => {
            return (
              ...
              SUSHI_ADDRESS[pair.chainId].toLowerCase()
            );
          });
          if (sushiIncentive) {
            spent =
              Number(sushiIncentive.rewardPerDay) *
              Number(sushiPriceUSD) *
              days;
          }
        }
        //process revenues
        const previousFees = Number(
          pastPairs.find((p) => p.id === pair.id)?.feesUSD
        );
        const revenue =
          (previousFees
            ? Number(pair.feesUSD) - previousFees
            : Number(pair.feesUSD)) / 6;
        const previousVolume = Number(
          pastPairs.find((p) => p.id === pair.id)?.volumeUSD
        );
        const volume = previousVolume
          ? Number(pair.volumeUSD) - previousVolume
