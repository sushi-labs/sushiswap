...
import {
  deprecated_TokensAndBundleQuery,
  getBuiltGraphSDK,
...
export async function bentobox(args: Arguments) {
      ...
      Object.entries(EXCHANGE_SUBGRAPH_NAME)
        ...
        .map(([chainId, subgraphName]) => {
          return getBuiltGraphSDK({
            ...
            subgraphName,
            subgraphHost:
              SUBGRAPH_HOST[Number(chainId) as keyof typeof SUBGRAPH_HOST],
          })
            .deprecated_TokensAndBundle({
              ...
              where: {
                id_in: rebases
                  .filter(
                    (rebase) => Number(rebase.chainId) === Number(chainId)
                  )
  ...
  rebases
    .reduce<[number, string | undefined, number, number][]>(
      (previousValue, rebase) => {
        const tokenPrice: deprecated_TokensAndBundleQuery =
          tokenPrices[rebase.chainId];
        const ethPrice = tokenPrice?.bundle?.ethPrice;
        if (!tokenPrice || !tokenPrice?.tokens?.length || !ethPrice) {
          return previousValue;
        }
        const token = tokenPrice.tokens.find(
          (token) => token.id.toLowerCase() === rebase.id.toLowerCase()
        );
        return [
          ...previousValue,
          [
    ...
    .slice(0, 25)
    .forEach((a) =>
      table.push([a[0], a[1], formatNumber(a[2]), formatNumber(a[3])])
    );
