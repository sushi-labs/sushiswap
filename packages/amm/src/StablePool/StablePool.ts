...
export class StablePool implements Pool {
  ...
  private _f(x0: bigint, y: bigint) {
    ...
    return (
      (x0 * ((((y * y) / getBigInt(1e12)) * y) / getBigInt(1e12))) / getBigInt(1e12) +
      (((((x0 * x0) / getBigInt(1e12)) * x0) / getBigInt(1e12)) * y) / getBigInt(1e12)
    )
  }
  ...
  private _d(x0: bigint, y: bigint) {
    ...
    return (
      (3n * x0 * ((y * y) / getBigInt(1e12))) / getBigInt(1e12) + (((x0 * x0) / getBigInt(1e12)) * x0) / getBigInt(1e12)
    )
  }
  ...
  private _get_y(x0: bigint, xy: bigint, y: bigint): bigint {
    for (let i = 0; i < 255; i++) {
      ...
      if (k < xy) {
        const dy = ((xy - k) * getBigInt(1e12)) / this._d(x0, y)
        y = y + dy
      } else {
        const dy = ((k - xy) * getBigInt(1e12)) / this._d(x0, y)
        y = y - dy
      }
      if (y > yPrev) {
        if (y - yPrev <= 1n) {
  ...
  private _getOutputAmount(inputAmount: Amount<Token>, reserve0: bigint, reserve1: bigint) {
    const adjustedReserve0 = (reserve0 * getBigInt(1e12)) / this.decimals0
    const adjustedReserve1 = (reserve1 * getBigInt(1e12)) / this.decimals1
    const feeDeductedAmountIn = inputAmount.quotient - inputAmount.quotient * getBigInt(this.fee)
    ...
    if (inputAmount.currency.equals(this.token0)) {
      ...
      const x0 = adjustedReserve0 + (feeDeductedAmountIn * getBigInt(1e12)) / this.decimals0
      const y = this._get_y(x0, xy, adjustedReserve1)
      const dy = adjustedReserve1 - y
      return (dy * this.decimals1) / getBigInt(1e12)
    } else {
      ...
      const x0 = adjustedReserve1 + (feeDeductedAmountIn * getBigInt(1e12)) / this.decimals1
      const y = this._get_y(x0, xy, adjustedReserve0)
      const dy = adjustedReserve0 - y
      return (dy * this.decimals0) / getBigInt(1e12)
    }
  }
  ...
  public getNonOptimalMintFee(amount0: bigint, amount1: bigint, reserve0: bigint, reserve1: bigint): [bigint, bigint] {
    ...
    if (amount1Optimal <= amount1) {
      return [ZERO, (getBigInt(this.fee) * (amount1 - amount1Optimal)) / (2n * 10000n)]
    } else {
      const amount0Optimal = (amount1 * reserve0) / reserve1
      return [(getBigInt(this.fee) * (amount0 - amount0Optimal)) / (2n * 10000n), ZERO]
    }
  }
  ...
  private _k(x: bigint, y: bigint): bigint {
    const a = (x * y) / getBigInt(1e12)
    const b = (x * x) / getBigInt(1e12) + (y * y) / getBigInt(1e12)
    return (a * b) / getBigInt(1e12)
  }
  ...
  public getAmountOut(amountIn: bigint, reserveAmountIn: bigint, reserveAmountOut: bigint): bigint {
    const amountInWithFee = amountIn * (this.MAX_FEE - getBigInt(this.fee))
