...
const SlipstreamSugarHelperAbi = [
  {
    inputs: [
...
// diffecence from classic V3:
//   getPopulatedTicks function (takes startTick, not a word index) instead of getPopulatedTicksInWord
export class AerodromeSlipstreamWordLoadManager extends EventEmitter {
  poolAddress: Address
  poolSpacing: number
      ...
      try {
        while (this.downloadQueue.length > 0) {
          ...
          >(
            ...
            SlipstreamSugarHelperAbi,
            'getPopulatedTicks',
            [this.poolAddress, wordIndex * this.poolSpacing * 256],
          )
