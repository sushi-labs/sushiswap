...
export type QualityCheckerCallBackArg = {
  ethalonPool: AlgebraPoolWatcher
  correctPool?: AlgebraPoolWatcher
  status: PoolSyncState
}

export type QualityCheckerCallBack = (arg: QualityCheckerCallBackArg) => boolean

export class AlgebraQualityChecker {
  readonly checkAfterLogsNumber: number
  readonly callBack
  checkingPools: Map<string, AlgebraPoolWatcher> = new Map()
  ...
  async check(
    pool: AlgebraPoolWatcher,
    newPool: AlgebraPoolWatcher,
  ): Promise<[AlgebraPoolWatcher | undefined, PoolSyncState, number, number]> {
    try {
  ...
  processLog(l: Log, pool: AlgebraPoolWatcher) {
    ...
    if (checkingPool) checkingPool.processLog(l)
      ...
      if (counter < this.checkAfterLogsNumber)
        ...
        const newPool = new AlgebraPoolWatcher(
          pool.provider,
          pool.address,
