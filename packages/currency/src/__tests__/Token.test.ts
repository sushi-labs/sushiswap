...
describe('Token', () => {
  ...
  describe('#constructor', () => {
    ...
    it('fails with negative decimals', () => {
      expect(() => new Token({ chainId: 3, address: ADDRESS_ONE, decimals: -1 }).address).toThrow('DECIMALS')
    })
    it('fails with 256 decimals', () => {
      expect(() => new Token({ chainId: 3, address: ADDRESS_ONE, decimals: 256 }).address).toThrow('DECIMALS')
    })
    it('fails with non-integer decimals', () => {
      expect(() => new Token({ chainId: 3, address: ADDRESS_ONE, decimals: 1.5 }).address).toThrow('DECIMALS')
    })
  })
