...
const ANKR_API_KEY: Record<number, string | undefined> = {
  [ChainId.ETHEREUM]:
    process.env.ANKR_ETHEREUM_ID || process.env.NEXT_PUBLIC_ANKR_ETHEREUM_ID,
  [ChainId.POLYGON]:
    process.env.ANKR_POLYGON_ID || process.env.NEXT_PUBLIC_ANKR_POLYGON_ID,
  [ChainId.ARBITRUM]:
    process.env.ANKR_ARBITRUM_ID || process.env.NEXT_PUBLIC_ANKR_ARBITRUM_ID,
  [ChainId.OPTIMISM]:
    process.env.ANKR_OPTIMISM_ID || process.env.NEXT_PUBLIC_ANKR_OPTIMISM_ID,
};
...
export function getProvider(chainId: ChainId) {
  ...
  if (ALCHEMY_ENABLED_CHAINS.includes(chainId)) {
    ...
    if (!alchemyKey)
      throw new Error(
        "ALCHEMY_ID || NEXT_PUBLIC_ALCHEMY_ID is required for provider"
      );
    const name = getAlchemyChainName(chainId);
    providerCache[chainId] = new AlchemyProvider(
      name,
      ALCHEMY_API_KEY[chainId]
    );
  } else if (INFURA_ENABLED_CHAINS.includes(chainId)) {
    const infuraKey =
      process.env.INFURA_PROJECT_ID || process.env.NEXT_PUBLIC_INFURA_ID;
    if (!infuraKey)
      throw new Error(
        "INFURA_PROJECT_ID || NEXT_PUBLIC_INFURA_ID is required for provider"
      );
    ...
    if (!ankrKey)
      throw new Error(
        "PUBLIC_ANKR_ID || NEXT_PUBLIC_ANKR_ID is required for provider"
      );
...
export function getWebsocketProvider(chainId: ChainId) {
  ...
  if (ALCHEMY_ENABLED_CHAINS.includes(chainId)) {
    ...
    if (!alchemyKey)
      throw new Error(
        "ALCHEMY_API_KEY || NEXT_PUBLIC_ALCHEMY_API_KEY is required for provider"
      );
    const name = getAlchemyChainName(chainId);
    websocketProviderCache[chainId] = new AlchemyWebSocketProvider(
      name,
      ALCHEMY_API_KEY[chainId]
    );
  } else if (INFURA_ENABLED_CHAINS.includes(chainId)) {
    ...
    if (!infuraKey)
      throw new Error(
        "INFURA_PROJECT_ID || process.env.NEXT_PUBLIC_INFURA_ID is required for provider"
      );
    const name = getInfuraChainName(chainId);
    websocketProviderCache[chainId] = new InfuraWebSocketProvider(
      name,
      infuraKey
    );
  }
