...
export function useBentoBoxApproveCallback({
  ...
  const approveBentoBox = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      console.error(
        "Error approving BentoBox, attempting regular approval instead",
        e
      );
  ...
  return useMemo(
    () => [approvalState, signature, approveBentoBox],
    [approvalState, approveBentoBox, signature]
  );
}
