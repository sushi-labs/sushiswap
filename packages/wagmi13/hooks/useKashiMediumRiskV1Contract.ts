...
export const getKashiMediumRiskV1ContractConfig = (
  chainId: number | undefined
) => ({
  address: (kashiExports[
    chainId as unknown as keyof Omit<typeof kashiExports, "31337">
  ]?.[0]?.contracts?.KashiPairMediumRiskV1?.address ?? "") as Address,
  abi:
    kashiExports[
      chainId as unknown as keyof Omit<typeof kashiExports, "31337">
    ]?.[0]?.contracts?.KashiPairMediumRiskV1?.abi ?? [],
});
