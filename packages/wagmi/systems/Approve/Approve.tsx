...
const reducer = (state: State, action: ApprovalAction) => {
  switch (action.type) {
    case "update": {
      ...
      const isUnknown = approvals.some(
        (el) => el?.[0] === ApprovalState.UNKNOWN
      );
      const isPending = approvals.some(
        (el) => el?.[0] === ApprovalState.PENDING
      );
      const isApproved = approvals.every(
        (el) =>
          el?.[0] === ApprovalState.APPROVED ||
          (el?.[0] === ApprovalState.PENDING && el?.[2])
      );
...
const Controller: FC<Props> = ({
  ...
  const initialized =
    state.approvals.length ===
      Children.toArray(components.props.children).length &&
  ...
  const children = useMemo(() => {
    return cloneElement(
      ...
      Children.map(components.props.children, (component, index) => {
        const valid = isValidElement<TokenApproveButton | BentoApproveButton>(
          component
        );
        if (valid) {
  ...
  const button = useMemo(() => {
    const index = state.approvals.findIndex(
      (el) => el?.[0] === ApprovalState.NOT_APPROVED
    );
    if (
      state.approvals &&
      index !== undefined &&
