...
export const useTrade: UseTrade = ({
  ...
  if (ammEnabled && !isSushiSwapV2ChainId(chainId))
    throw new Error(
      `ChainId Error: SushiSwapV2 is not available on ${chainName[chainId]} and ammEnabled is enabled.`,
    )

  // TODO: Use trident chainId instead of Bento
  if (tridentEnabled && !isBentoBoxChainId(chainId))
    throw new Error(
      `ChainId Error: BentoBox is not available on ${chainName[chainId]} and tridentEnabled is enabled.`,
    )
  ...
  } = useSushiSwapV2Pools(chainId as SushiSwapV2ChainId, currencyCombinations, {
    enabled: ammEnabled,
  })
    ...
    if (
      ...
      if (tradeType === TradeType.EXACT_INPUT) {
        if (
          ...
          getTridentConstantPoolFactoryContract(chainId).address
        ) {
