...
type UseWrapCallback = (
  params: UseWrapCallbackParams
) => ReturnType<typeof useSendTransaction>;

export const useWrapCallback: UseWrapCallback = ({
  chainId,
  wrapType,
  amount,
  onSuccess,
        ...
        onSuccess({
          ...
          summary: {
            ...
            completed: `Successfully ${
              wrapType === WrapType.Wrap ? "wrapped" : "unwrapped"
            } ${amount.toSignificant(6)} ${amount.currency.symbol}`,
  ...
  return useSendTransaction({
    ...
    onSettled,
    enabled:
      !!contract && chainId && address && amount && amount?.greaterThan(ZERO),
  });
};
