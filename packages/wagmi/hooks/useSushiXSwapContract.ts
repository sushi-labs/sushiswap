...
export const getSushiXSwapContractConfig = (chainId: number | undefined) =>
  ({
    address: (chainId
      ? sushiXSwapExports[chainId.toString() as SushiXSwapChainId]?.[0]
          ?.contracts?.SushiXSwap?.address
      : "") as Address,
    abi: sushiXSwapAbi,
  } as const);

export function useSushiXSwapContract(
...
export function useSushiXSwapContractWithProvider(
  chainId: number | undefined
): ReturnType<typeof useContract> {
