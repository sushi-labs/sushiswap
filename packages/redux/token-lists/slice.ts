...
export function createTokenListsSlice(reducerPath: string): Slice<TokenListsState> {
  return createSlice({
    ...
    initialState,
    reducers: {
      pending: (state, action: PayloadAction<PendingPayload>) => {
      ...
      fulfilled: (state, action: PayloadAction<FulfilledPayload>) => {
      ...
      rejected: (state, action: PayloadAction<RejectedPayload>) => {
      ...
      enable: (state, action: PayloadAction<EnablePayload>) => {
        const url = action.payload
        if (!state.byUrl[url]) {
          state.byUrl[url] = NEW_TOKEN_LIST_STATE
        }
      ...
      disabled: (state, action: PayloadAction<DisablePayload>) => {
        const url = action.payload
        if (state.activeListUrls && state.activeListUrls.includes(url)) {
          state.activeListUrls = state.activeListUrls.filter((u) => u !== url)
        }
      ...
      accept: (state, action: PayloadAction<AcceptPayload>) => {
        const url = action.payload
