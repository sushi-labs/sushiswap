...
export const initialState: TokenListsState = {
  ...
  byUrl: {
    ...DEFAULT_LIST_OF_LISTS.reduce<Mutable<TokenListsState["byUrl"]>>(
      (memo, listUrl) => {
...
const reducers = {
  ...
  fulfilled: (
    state: TokenListsState,
    action: PayloadAction<FulfilledPayload>
  ) => {
  ...
  rejected: (
    state: TokenListsState,
    action: PayloadAction<RejectedPayload>
  ) => {
...
export function createTokenListsSlice(
  reducerPath: string
): Slice<TokenListsState> {
  return createSlice({
    name: reducerPath,
    initialState,
    reducers,
...
export type TokenListsActions = ReturnType<
  typeof createTokenListsSlice
>["actions"];
