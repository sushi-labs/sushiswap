...
type UseCustomTokensReturn = [
  Record<string, Record<string, Token>>,
  {
    ...
    removeCustomToken(
      payload: Pick<TokenAsObject, "address" | "chainId">
    ): void;
  }
];
...
export const useAllCustomTokens: UseAllCustomTokens = (context) => {
  ...
  const { customTokens } = useSelector(
    (state: WithStorageState) => state[reducerPath]
  );
  ...
  const addCustomToken = useCallback(
    ({ symbol, address, chainId, name, decimals }: TokenAsObject) => {
      dispatch(
        actions.addCustomToken({ symbol, address, chainId, name, decimals })
      );
    },
    [actions, dispatch]
  );
  ...
  const tokens = useMemo(() => {
    ...
    return chainIds.reduce<Record<string, Record<string, Token>>>(
      (acc, chainId) => {
        acc[chainId] = Object.entries(customTokens[+chainId]).reduce<
          Record<string, Token>
