...
export const _pairsByChainIds = async (root = {}, args, context, info) => {
    return Promise.all([
        ...args.chainIds
            .filter((chainId) => SUSHISWAP_ENABLED_NETWORKS.includes(chainId))
            .map((chainId) => context.SushiSwap.Query.pairs({
            root,
            args: {
                ...args,
        ...
        ...args.chainIds
            .filter((chainId) => TRIDENT_ENABLED_NETWORKS.includes(chainId))
            .map((chainId) => context.Trident.Query.pairs({
            root,
            args: {
                ...args,
                where: {
...
export const pairsByChainIds = async (root, args, context, info) => {
    return _pairsByChainIds(root, args, context, info);
};
//# sourceMappingURL=pairsByChainIds.js.map