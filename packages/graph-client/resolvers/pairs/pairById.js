// @ts-nocheck
import { SUSHISWAP_ENABLED_NETWORKS, SUSHISWAP_SUBGRAPH_URL, SUSHISWAP_V3_ENABLED_NETWORKS, SUSHISWAP_V3_SUBGRAPH_URL, TRIDENT_ENABLED_NETWORKS, TRIDENT_SUBGRAPH_URL, } from '@sushiswap/graph-config';
...
const transformV3PoolToPair = (pool, chainId) => ({
    id: `${chainId}:${pool.id}`,
    chainId,
    address: pool.id,
    chainId: chainId,
...
export const pairById = async (root, args, context, info) => {
    const now = Date.now();
    const [chainShortName, address] = args.id.split(':');
    const chainId = chainShortNameToChainId[chainShortName];
    const [{ oneDayBlocks: [oneDayBlock], }, { twoDayBlocks: [twoDayBlock], }, { oneWeekBlocks: [oneWeekBlock], },] = await Promise.all([
        sdk.OneDayBlocks({ chainIds: [chainId] }),
        sdk.TwoDayBlocks({ chainIds: [chainId] }),
        sdk.OneWeekBlocks({ chainIds: [chainId] }),
    ]);
    const fetchSushiSwapPair = async (block) => context.SushiSwap.Query.pair({
        root,
    ...
    const fetchTridentPair = async (block) => context.Trident.Query.pair({
        root,
        args: { ...args, id: address.toLowerCase(), block },
        context: {
            ...context,
    ...
    const fetchSushiSwapV3Pair = async (block) => {
        const sdk = getBuiltGraphSDK({
...
//# sourceMappingURL=pairById.js.map