...
export const crossChainUserWithFarms: QueryResolvers["crossChainUserWithFarms"] =
  async (
    root,
    args,
    context,
    ...
    const [farms, { liquidityPositions }, { crossChainChefUser: chefUser }] =
      await Promise.all([
        getFarms(),
    ...
    const stakedLiquidityPositionChainIds = chefUser.map(
      (chefUser) => chefUser?.chainId
    );
    ...
    const unstakedLiquidityPositionChainIds = liquidityPositions.map(
      ({ chainId }) => chainId
    );
    ...
    const chainIds = Array.from(
      new Set([
        ...stakedLiquidityPositionChainIds,
        ...unstakedLiquidityPositionChainIds,
      ])
    );
        ...
        sdk.PairsByChainId({
          ...
          where: {
            id_in: [
              ...liquidityPositions
                .filter(
                  (liquidityPosition) => liquidityPosition.chainId === chainId
                )
                .map(({ id }) => id.split("-")[0]),
              ...chefUser
                .filter(
                  (chefUser) =>
                    chefUser.chainId === chainId &&
                    typeof chefUser?.pool?.pair === "string"
                )
    ...
    return Array.isArray(pairs)
      ? pairs
          .filter((pair) => {
            ...
            return (
              id in _unstakedLiquidityPositionBalances ||
              id in stakedLiquidityPositionBalances
            );
          })
          .map((pair) => {
            const id = `${pair.chainId}:${pair.id}`;
            ...
            const balance =
              Number(unstakedBalance ?? 0) + Number(stakedBalance ?? 0);
            const valueUSD =
              (balance / Number(pair?.liquidity ?? 0)) *
              Number(pair?.liquidityUSD ?? 0);
