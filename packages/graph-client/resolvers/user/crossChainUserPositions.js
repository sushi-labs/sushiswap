...
export const crossChainUserPositions = async (root, args, context, info) => {
    const sdk = getBuiltGraphSDK();
    const [{ liquidityPositions }, { crossChainChefUser: chefUser }] = await Promise.all([
    ...
    const stakedBalances = Object.fromEntries(chefUser.map((chefUser) => [
        `${chefUser.chainId}:${chefUser?.pool?.pair}`,
        chefUser?.amount,
    ]));
    // Subgraph is unreliable currently, have to fetch from the chain directly
    const unstakedBalances = await fetchBalances(liquidityPositions.map((lp) => ({
        token: lp.id.split('-')[0],
        chainId: lp.chainId,
        user: args.id,
    })));
...
//# sourceMappingURL=crossChainUserPositions.js.map