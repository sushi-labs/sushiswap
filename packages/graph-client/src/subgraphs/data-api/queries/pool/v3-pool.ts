...
export const V3PoolQuery = graphql(
  `
  query V3Pool($address: String!, $chainId: Int!) {
...
export async function getV3Pool(
  ...
  if (result.v3Pool) {
    ...
    return {
      ...
      poolHourData: pool.hourBuckets.filter((bucket) => bucket !== null),
      poolDayData: pool.dayBuckets.filter((bucket) => bucket !== null),
...
export type V3Pool = Awaited<ReturnType<typeof getV3Pool>>
