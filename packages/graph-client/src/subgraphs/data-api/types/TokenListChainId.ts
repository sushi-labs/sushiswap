// This file is auto-generated by scripts/update-data-api-types.ts
export const TokenListChainIds = [
  42161, 42170, 43114, 8453, 288, 56, 42220, 1, 250, 100, 137, 534352, 314, 10,
  59144, 2046399126, 30, 146, 43111, 747474, 81457, 4663, 5042, 11155111,
  129399, 737373, 130, 480, 25, 5000, 324, 169, 34443, 167000, 810180, 33139,
  999, 80094, 9745, 143, 4326, 196, 1101, -5, -4,
] as const
export type TokenListChainId = (typeof TokenListChainIds)[number]
export function isTokenListChainId(value: number): value is TokenListChainId {
  return TokenListChainIds.includes(value as TokenListChainId)
}
