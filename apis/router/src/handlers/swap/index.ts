...
const networks: Map<ChainId, Network> = new Map()
  ...
  return async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (network === undefined) {
        ...
        return res.status().send(`Network ${chainId} is not supported`)
      }

      // Timing optimization: try to take tokens sync first - to avoid async call if tokens are known
      let tokensAreKnown = true
      const _tokenInL = _tokenIn.toLowerCase()
      const _tokenOutL = _tokenOut.toLowerCase()
      let tokenIn =
        _tokenInL === '0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee'
          ? Native.onChain(chainId)
          : network.tokenMap.get(_tokenInL)
      let tokenOut =
        _tokenOutL === '0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee'
          ? Native.onChain(chainId)
          : network.tokenMap.get(_tokenOutL)
      if (!tokenIn || !tokenOut) {
