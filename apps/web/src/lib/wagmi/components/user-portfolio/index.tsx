...
const ResponsivePortfolioWrapper: FC<{
	...
	return isSm ? (
		<Sheet open={isOpen} onOpenChange={(open) => handleAccountDrawer({ state: open })}>
		...
		<Dialog open={isOpen} onOpenChange={(open) => handleAccountDrawer({ state: open })}>
			<DialogTrigger asChild>{trigger}</DialogTrigger>
			<DialogContent variant="semi-opaque" hideClose className="!p-0 h-[calc(100%-55px)]">
				{content}
			</DialogContent>
		</Dialog>
	);
			...
			default:
				return (
					<>
						...
						<PortfolioDefaultView setView={setView} ensName={ensName} isENSNameLoading={isENSNameLoading} />
					</>
				);
		}
