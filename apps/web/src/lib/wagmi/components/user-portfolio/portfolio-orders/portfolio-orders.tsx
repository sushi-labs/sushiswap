...
import { Button, Select, SelectContent, SelectItem, SelectTrigger } from "@sushiswap/ui";
...
export const PortfolioOrders = () => {
	...
	const [completedOrderFilter, setCompletedOrderFilter] = useState(CompletedOrderType.All);
	const { handleAccountDrawer, subAccountTab } = useAccountDrawer();

	const handleTabChange = (newTab: OrdersTab) => {
		setTab(newTab);
		handleAccountDrawer({ state: true, params: { name: "subAccountTab", value: newTab } });
	};
	...
	return (
		<div className="flex flex-col gap-y-5 min-h-0">
					...
					<Select
						...
						<SelectContent className="!border !border-accent">
							{Object.values(CompletedOrderType).map((filter, idx) => (
								<SelectItem key={idx} value={filter} title={filter} className="text-base cursor-pointer">
									{filter}
								</SelectItem>
							))}
						</SelectContent>
			...
			<div className="flex flex-col gap-y-5 px-5 overflow-y-auto">{content}</div>
		</div>
	);
};
