...
export function useSearchTokensV2({
	...
	const customTokens = useMemo(() => {
		...
		return Object.values(_customTokens)
			.filter((t) => isTokenListV2ChainId(t.chainId) && chainIds?.includes(t.chainId))
			.map((t) => ({
				address: t.address,
	...
	const query = useInfiniteQuery({
		...
		queryFn: async ({ pageParam }) => {
			if (!chainIds || chainIds.length === 0) throw new Error("chainIds are required");
	...
	return useMemo(
		() => ({
			...
			priceMap: priceMap,
			hasMore: query.data?.pages[query.data.pages.length - 1].length === pagination.pageSize,
		}),
