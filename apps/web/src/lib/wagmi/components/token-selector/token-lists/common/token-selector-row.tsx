...
import { BrowserEvent, InterfaceElementName, InterfaceEventName, TraceEvent } from "@sushiswap/telemetry";
import {
	IconButton,
	SkeletonCircle,
	SkeletonText,
			...
			<div className="relative py-0.5 h-[64px]" style={style}>
				<div
					...
					<div className="flex items-center justify-between flex-grow gap-2 rounded cursor-pointer">
						<FavoriteButton currencyId={currency?.id} onClick={onPin} className="!px-0.5" />
						<div className="flex flex-row items-center flex-grow gap-4">
							{selected ? (
								<Badge
									...
									<div className="w-10 h-10">
										<Currency.Icon disableLink currency={currency} width={40} height={40} />
									</div>
								</Badge>
							) : (
								<div className="w-10 h-10">
							...
							<div className="flex flex-col items-start">
								<div className="flex gap-1">
									<span className="font-semibold text-primary">{currency.symbol}</span>
									{showWarning ? (
										<TooltipProvider>
