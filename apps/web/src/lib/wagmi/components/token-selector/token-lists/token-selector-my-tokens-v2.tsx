...
export function TokenSelectorMyTokensV2({
	...
	const myTokensChainIds = useMemo(
		() =>
			chainId && isTokenListV2ChainId(chainId) ? [chainId] : networkOptions.filter(isTokenListV2ChainId),
	...
	if (isError)
		return (
			<Shell>
				<div className="flex w-full justify-center py-3">An error has occurred.</div>
			</Shell>
		);
	...
	if (!data.balanceMap?.size)
		return (
			<Shell>
				<div className="flex w-full justify-center py-3">No balances found.</div>
			</Shell>
		);
