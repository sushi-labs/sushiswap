...
import { useSteerPositionAddDerivedInfo } from './steer-position-add-provider'
...
export const SteerPositionAddReviewModal: FC<
  SteerPositionAddReviewModalProps
> = ({ vault, onSuccess: _onSuccess, successLink, children }) => {
  ...
  const { approved } = useApproved(APPROVE_TAG_ADD_STEER)
                ...
                <List className="!pt-0">
                  <List.Control>
                    <List.KeyValue flex title="Network">
                      {getEvmChainById(vault.chainId).name}
                    </List.KeyValue>
                  </List.Control>
                </List>
                <List className="!pt-0">
