...
import { useSteerVaultReserves } from 'src/lib/steer/hooks/use-steer-vault-reserves'
...
export function useSteerPositionAddDerivedInfo({
  ...
  const [currencyA, currencyB] = useMemo(() => {
    ...
    return [new EvmToken(vault.token0), new EvmToken(vault.token1)]
  }, [vault])

  // currencies
  const currencies: { [_field in Field]: EvmCurrency } | undefined =
    useMemo(() => {
  ...
  const parsedAmounts: { [_ in Field]: Amount<EvmCurrency> } | undefined =
    useMemo(() => {
