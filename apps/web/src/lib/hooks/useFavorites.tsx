...
import { type TokenListV2ChainId, isTokenListV2ChainId } from "@sushiswap/graph-client/data-api";
...
export const useFavorites = () => {
	...
	const { supportedTokens, uniqueChainIds, unsupportedTokens, natives } = useMemo(() => {
		const tokens = Object.values(_pinnedTokens)
			...
			.map((i) => {
				...
				return {
					...
					isSupported:
						isTokenListV2ChainId(Number(chainId) as EvmChainId) && contractAddress !== NativeAddress,
					symbol: symbol,
				};
			})
			.filter((i) => i !== undefined);

		const uniqueChainIds = Array.from(new Set(tokens.map((token) => Number(token.chainId)))).filter(
		...
		const unsupportedTokens = tokens
			.filter((token) => !token.isSupported && token.address !== NativeAddress)
			.map((token) => ({
				address: token.address,
		...
		const natives = tokens
			...
			.map((token) => ({
				address: EvmNative.fromChainId(token.chainId as EvmChainId).wrap().address,
				chainId: token.chainId,
				symbol: EvmNative.fromChainId(token.chainId as EvmChainId).wrap().symbol,
			}));
	...
	const favorites = useMemo(() => {
		...
		unsupportedTokens.forEach((token) => {
			const priceUsd = _nativeTokens?.priceMap?.get(`${token.chainId}:${token.address}`) ?? 0;
			const balance =
				_nativeTokens?.balanceMap?.get(`${token.chainId}:${token.address}`)?.toString({ fixed: 6 }) ?? "0";
		...
		const nativesOnList = _nativeTokens.tokens.filter((token) => {
			return natives?.some((nativeToken) => nativeToken.chainId === token.chainId && token.isNative);
		});

		nativesOnList.forEach((token) => {
			if (token.isNative) {
				const _native = EvmNative.fromChainId(token.chainId);
				const priceUsd = _nativeTokens?.priceMap?.get(`${token.chainId}:NATIVE`) ?? 0;
				const balance =
					_nativeTokens?.balanceMap?.get(`${token.chainId}:NATIVE`)?.toString({ fixed: 6 }) ?? "0";
				const balanceUsd = priceUsd * Number(balance);
				favArr.push({
					//need to typecast here, need to satify SearchToken chainId type
					chainId: token.chainId as TokenListV2ChainId,
