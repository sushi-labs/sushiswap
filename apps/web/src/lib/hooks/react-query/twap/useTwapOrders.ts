...
export const usePersistedOrdersStore = ({ account }: TwapOrdersStoreParams) => {
		...
		(
			...
			try {
				tradeDollarValueIn = srcTokenUsdPrice
					? Number(formatUnits(BigInt(params[3]), srcToken.decimals)) * srcTokenUsdPrice
					: 0;
			} catch (_error) {
			...
			queryClient.setQueryData(queryKey, (orders?: TwapOrder[]) => {
				const _order = {
					...order,
					status: OrderStatus.Open,
					fillDelayMs: getOrderFillDelayMillis(order, TwapSDK.onNetwork(chainId).config),
				};
	...
	const addCancelledOrderId = useCallback(
		(chainId: TwapSupportedChainId, orderId: number) => {
			...
			if (!cancelledOrderIds.includes(orderId)) {
				...
				queryClient.setQueryData(["twap-orders", chainId, account], (orders?: TwapOrder[]) => {
					if (!orders) return [];
	...
	const deleteCreatedOrder = useCallback(
		(chainId: TwapSupportedChainId, id: number) => {
			const { ordersKey } = getKeys(chainId);
			const orders = getCreatedOrders(chainId).filter((order) => order.id !== id);
	...
	const deleteCancelledOrderId = useCallback(
		(chainId: TwapSupportedChainId, orderId: number) => {
			const { cancelledOrderIdsKey } = getKeys(chainId);
			const cancelledOrderIds = getCancelledOrderIds(chainId).filter((id) => id !== orderId);
			localStorage.setItem(cancelledOrderIdsKey, JSON.stringify(cancelledOrderIds));
		},
...
const useTwapOrdersQuery = ({ chainIds, account, enabled = true }: TwapOrdersQueryParams) => {
	const config = useConfig();

	const { getCreatedOrders, getCancelledOrderIds, deleteCreatedOrder, deleteCancelledOrderId } =
	...
	return useQueries({
		queries: chainIds.map((chainId) => ({
			...
			queryFn: async ({ signal }: { signal: AbortSignal }) => {
				...
				const sdkOrders = await TwapSDK.onNetwork(chainId).getOrders(account, signal);
...
export const useTwapOrders = ({ chainIds, account, enabled = true }: TwapOrdersQueryParams) => {
		...
		return {
			...
			data: sortedOrders
				? {
						...
						[OrderStatus.Completed]: filterOrders(sortedOrders, OrderStatus.Completed),
						[OrderStatus.Expired]: filterOrders(sortedOrders, OrderStatus.Expired),
						[OrderStatus.Canceled]: filterOrders(sortedOrders, OrderStatus.Canceled),
				  }
				: undefined,
		};
