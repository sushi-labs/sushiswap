import { eqIgnoreCase, zeroAddress, getNetwork } from "@orbs-network/twap-sdk";
...
export const useParsedOrder = (order: TwapOrder) => {
  ...
  return useMemo(() => {
    ...
    const fills = order.fills?.map((fill) =>
      parseFill(order, fill, sellToken, buyToken)
    ) ?? [];
    ...
    return {
      ...
      fills,
      avgBuyTokenAmountPerChunk: fills.reduce((acc, fill) => acc + Number(fill.buyAmount), 0) /
      fills.length,
      avgBuyTokenUsdPerChunk: fills.reduce((acc, fill) => acc + Number(fill.buyAmountUsd), 0) /
      fills.length,
    };
