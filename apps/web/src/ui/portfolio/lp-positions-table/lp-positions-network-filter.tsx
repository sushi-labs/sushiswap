				...
				<Button
					...
					{!allSelected && networks && networks?.length > 0 ? (
						<>
							...
							<div>{networks.length > 1 ? ` +${networks.length - 1}` : null}</div>
							<button
								type="button"
								onClick={(e) => {
									e.stopPropagation();
						...
						{PLACEHOLDER_NETWORKS.map((_network) => {
							...
							return (
								<CommandItem
									...
									onSelect={onSelect}
									className={classNames("py-2 pr-2 hover:bg-[#0000000A] hover:dark:bg-[#FFFFFF0A]")}>
									<NetworkIcon type="circle" chainId={_network} width={20} height={20} className="mr-2" />
									{name}
									{isSelected ? (
