...
import { CardContent, CardHeader, SkeletonText, classNames } from "@sushiswap/ui";
...
export const BarChartGraph: FC<BarChartProps> = ({ chart, period }) => {
	...
	const DEFAULT_OPTION = useMemo<EChartOption>(
		() => ({
            ...
            <span class="text-sm dark:text-slate-50 text-gray-900 font-medium">${
							chart === BarChartType.APR ? formatPercent(params[0].value) : formatNumber(params[0].value)
						}</span>
	...
	return (
		<>
			...
			<CardContent>
				{isLoading ? (
					<SkeletonBox className={classNames("h-[200px] w-full dark:via-slate-800 dark:to-slate-900")} />
				) : (
					<ReactEchartsCore echarts={echarts} option={DEFAULT_OPTION} style={{ height: 200 }} />
				)}
			</CardContent>
		</>
	);
