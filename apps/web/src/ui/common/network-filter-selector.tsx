...
export const NetworkFilterSelector = ({
  ...
  selectedNetworks: ChainId[] | undefined
  setNetworks: (value: ChainId[]) => void
  defaultNetworks: ChainId[]
  className?: string
}) => {
  const allSelected = selectedNetworks?.length === defaultNetworks.length

  const onSelect = (value: string) => {
    const _network = value.split('__')[1]
    const network = +_network as ChainId
    const newNetworks = selectedNetworks?.includes(network)
    ...
    <PopoverDrawer
      ...
      trigger={
        <Button
          ...
          className={classNames(
            ...
            !allSelected && selectedNetworks && selectedNetworks?.length > 0
        ...
        >
          {!allSelected && selectedNetworks && selectedNetworks?.length > 0 ? (
            <>
              <span>Network: </span>
              <div>{getNetworkName(selectedNetworks[0])}</div>
              <div>
      ...
      content={
        <Command className="flex items-center">
          ...
          {selectedNetworks && selectedNetworks?.length > 1 ? (
          ...
          <CommandGroup className="!overflow-x-hidden !overflow-y-scroll scroll max-h-[300px]">
            {defaultNetworks.map((_network) => {
              const name = getNetworkName(_network)
              const isSelected = selectedNetworks?.includes(_network)
