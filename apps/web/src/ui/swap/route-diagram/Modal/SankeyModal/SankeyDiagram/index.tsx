...
import {
  applyOpacityEffects,
  sankeyHoverOpacityLevels,
  transformRouteDataD3,
} from './helpers'
import {
...
export const SankeyDiagram: React.FC<SankeyChartProps> = ({
  ...
  `)
    ...
    const tooltipContainer = select(sankeyRef.current).select(
      '#tooltipContainer',
    )
    const tooltipContentContainer = select(sankeyRef.current).select(
      '#tooltipContent',
    )
    /**
    ...
    const nodeGroup = svg
      ...
      .data(nodes)
      .join('g')
      .attr('id', (d: NodeD3Custom) =>
        d.hide && d.pathkey ? d.pathkey : d.symbol,
      )
    ...
    nodeGroup
      ...
      .attr('class', (d: NodeD3Custom) =>
        d.hide ? hiddenNodeClass : nodeClass,
      )
      //filter and 1px inset stroke help complete the Shadow25 effect
      .attr('filter', (d: NodeD3Custom) =>
        d.hide ? '' : 'url(#box-shadow-25)',
      )
      .attr('stroke', (d: NodeD3Custom) =>
        d.hide ? 'transparent' : 'rgba(0,0,0,0.06)',
      )
      ...
      .attr('width', (d: NodeD3Custom) =>
        Math.floor(Math.ceil(d.x1) - Math.floor(d.x0)),
      )
      .attr('fill', (d: NodeD3Custom) =>
        d.hide ? `url(#gradient-${d.pathkey})` : 'white',
      )
      .attr('fill-opacity', (d: NodeD3Custom) =>
        d.hide ? sankeyHoverOpacityLevels.default : 1,
      )
      .on('mousemove', (event: MouseEvent | TouchEvent, d: NodeD3Custom) => {
        ...
        if (
          currentSubRoute.sourceLink.pathkey !==
          selectedSubRoute?.sourceLink.pathkey
        ) {
        ...
        const toolTipOverflowsOnBottom =
          tooltipY + TOOLTIP_HEIGHT > svgRect.height
  ...
  return (
    <div
      ref={sankeyRef}
      className={classNames(diagramContainerClass, className)}
    >
      <div
        id="tooltipContainer"
...
export function handleOnMouseOrTouchEvent({
  ...
  if (
    currentSubRoute.sourceLink.pathkey !== selectedSubRoute?.sourceLink.pathkey
  ) {
    selectedSubRoute = currentSubRoute
    tooltipContentContainer.html(
      `${renderToStaticMarkup(<SankeyTooltip {...currentSubRoute} />)}`,
    )
  }
  const svgRect = sankeyRef.current.getBoundingClientRect()
  let tooltipX
  ...
  const toolTipOverflowsOnRight =
    tooltipX + TOOLTIP_WIDTH > svgRect.width - TOOLTIP_WIDTH
  const toolTipOverflowsOnBottom =
    tooltipY + TOOLTIP_HEIGHT > svgRect.height - TOOLTIP_HEIGHT
  if (toolTipOverflowsOnRight) {
    tooltipX = tooltipX - TOOLTIP_WIDTH
  }
  if (toolTipOverflowsOnBottom) {
