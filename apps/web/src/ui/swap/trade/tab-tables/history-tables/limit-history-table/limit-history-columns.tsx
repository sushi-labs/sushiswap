...
export const DATE_COLUMN: ColumnDef<TwapOrder> = {
	...
	cell: ({ row }) => (
		<span className="whitespace-nowrap">{format(new Date(row.original.createdAt), "dd/MM/yy h:mm aa")}</span>
	),
};
...
export const VALUE_PNL_COLUMN: ColumnDef<TwapOrder> = {
	...
	cell: ({ row }) => {
		const { sellTokenTotalUsdValue, profitAndLoss } = useParsedOrder(row.original);
		return (
...
export const FILLED_COLUMN: ColumnDef<TwapOrder> = {
	...
	cell: ({ row }) => {
		const { buyToken, buyTokenFilledAmount, filledPercentage, buyTokenExpectedAmount } = useParsedOrder(
			row.original
		);
		...
		return (
			<div className="flex items-center gap-2">
				<span>
					{formatNumber(buyTokenFilledAmount)} / {formatNumber(Number(buyTokenExpectedAmount))}{" "}
					{buyToken.symbol}
				</span>
