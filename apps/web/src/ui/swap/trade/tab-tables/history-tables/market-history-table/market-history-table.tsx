...
import { TempChainIds, useRecentSwaps } from "src/lib/hooks/react-query/recent-swaps/useRecentsSwaps";
...
export const MarketTable = () => {
	...
	const priceCol = useMemo(() => getPriceUsdColumn(showInUsd, setShowInUsd), [showInUsd]);

	const COLUMNS = useMemo(
		() => [BUY_COLUMN, SELL_COLUMN, CHAIN_COLUMN, VALUE_PNL_COLUMN, priceCol, TX_HASH_COLUMN, DATE_COLUMN],
		[priceCol]
	);
	...
	const rowData = useMemo(() => {
		...
		return recentSwaps.map((swap, i) => {
			const { tokenIn, tokenOut, amountIn, amountInUSD, amountOut, amountOutUSD, totalPnl, time } = swap;
