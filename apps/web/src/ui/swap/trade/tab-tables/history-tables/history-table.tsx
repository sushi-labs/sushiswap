import { Button, Card, Tabs, TabsContent, TabsList, TabsTrigger, classNames } from "@sushiswap/ui";
	...
	return (
		<Card className="overflow-hidden border-none !shadow-none md:p-3 xl:bg-red-white dark:md:bg-slate-800 rounded-t-none xl:rounded-lg">
			<Tabs
					...
					{TABS.map((tab) => (
						<TabsTrigger
							...
							<Button
								...
								className={classNames(tab.value === currentTab ? "!bg-slate-200 dark:!bg-slate-750" : "")}>
								{tab.label}
							</Button>
						</TabsTrigger>
					))}
				...
				<TabsContent value={TABS[0].value} className="border border-accent md:border-none rounded-xl !mt-0">
					<MarketTable />
				</TabsContent>
				<TabsContent value={TABS[1].value} className="border border-accent md:border-none rounded-xl !mt-0">
					<LimitOrdersHistoryTable />
				</TabsContent>
				<TabsContent value={TABS[2].value} className="border border-accent md:border-none rounded-xl !mt-0">
