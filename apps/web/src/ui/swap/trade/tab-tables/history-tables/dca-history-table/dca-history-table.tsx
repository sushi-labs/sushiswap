...
import { type TwapOrder, getTwapDcaOrders } from "src/lib/hooks/react-query/twap";
...
export const DCAOrdersHistoryTable = () => {
	...
	const data = useMemo(() => {
		return getTwapDcaOrders(orders).filter((order) => order.status !== OrderStatus.Open);
	}, [orders]);

	const avgPriceColumn = useMemo(() => getAvgPriceColumn(showInUsd, setShowInUsd), [showInUsd]);
	...
	const MOBILE_ACTION_COLUMN = useMemo(() => makeActionColumn(setSelectedRowId), []);
	...
	return (
		<>
			<DCAOrderDetailsModal isOpen={isOpen} onOpenChange={setIsOpen} order={selectedOrder} />
			<InfiniteScroll
