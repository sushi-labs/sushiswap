...
import { TWAP_SUPPORTED_CHAIN_IDS, type TwapSupportedChainId, isTwapSupportedChainId } from "src/config";
...
const filterByPair = (orders?: TwapOrder[], token0?: Type, token1?: Type) => {
	...
	return orders.filter((order) => {
		return (
			...
			(eqIgnoreCase(order?.srcTokenAddress, dstAddress) && eqIgnoreCase(order?.srcTokenAddress, srcAddress))
		);
	});
};
...
const Context = createContext<TradeTablesContextType>({} as TradeTablesContextType);
export const TradeTablesProvider = ({ children }: { children: React.ReactNode }) => {
	return (
		<DerivedstateSimpleSwapProvider>
...
const Content = ({ children }: { children: React.ReactNode }) => {
	...
	const {
		...
		chainIds,
		account: "0xFa7768aDDb25AA83BfA5c257A443D1c730178c7c",
		enabled: true,
	});
	...
	const orders = useMemo(() => {
		return showCurrentPairOnly ? filterByPair(data?.ALL ?? [], token0, token1) ?? [] : data?.ALL ?? [];
