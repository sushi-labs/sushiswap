...
import { type TwapOrder, getTwapLimitOrders } from "src/lib/hooks/react-query/twap";
...
export const LimitOrdersTable = () => {
	...
	const priceCol = useMemo(() => getPriceColumn(showInUsd, setShowInUsd), [showInUsd]);

	const LIMIT_ORDER_COLUMNS: ColumnDef<TwapOrder>[] = useMemo(
		() => [BUY_COLUMN, SELL_COLUMN, CHAIN_COLUMN, VALUE_PNL_COLUMN, priceCol, TIME_COLUMN, ACTION_COLUMN],
		[priceCol]
	);

	const { orders, ordersLoading } = useTradeTablesContext();
	const data = useMemo(() => {
		return getTwapLimitOrders(orders).filter((it) => it.status === OrderStatus.Open);
	}, [orders]);
