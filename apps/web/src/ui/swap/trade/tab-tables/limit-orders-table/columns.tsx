...
import { type TwapOrder, useCancelOrder, useParsedOrder } from "src/lib/hooks/react-query/twap";
		...
		return (
			<div className="flex items-center gap-1 md:gap-2 min-w-[130px]">
				...
				<span className="whitespace-nowrap">
					{!buyTokenExpectedAmount ? "-" : `${formatNumber(buyTokenExpectedAmount)} ${buyToken.symbol}`}
				</span>
			</div>
		);
	},
	...
	cell: ({ row }) => {
		...
		return (
			<div className="flex items-center gap-1 md:gap-2">
				<Currency.Icon disableLink currency={sellToken} width={24} height={24} />
				<span className="whitespace-nowrap">
		...
		return (
			<div className="flex items-center gap-1 md:gap-2">
				<div className="dark:border-[#222137] border-[#F5F5F5] border rounded-[4px] overflow-hidden">
					<NetworkIcon type="square" chainId={chainInfo.id} className="w-3 h-3 md:w-5 md:h-5" />
				</div>
...
export const VALUE_PNL_COLUMN: ColumnDef<TwapOrder> = {
	...
	header: () => {
		return (
			<TooltipDrawer
				trigger={<span className="border-b border-dotted border-muted-foreground">Value / Est. PnL</span>}
				content={
					<p>
	...
	cell: ({ row }) => {
		const { profitAndLoss, sellTokenTotalUsdValue } = useParsedOrder(row.original);
...
export const getPriceColumn = (
	...
	cell: ({ row }) => {
		const { sellToken, usdPerChunk, sellTokenAmountPerChunk } = useParsedOrder(row.original);

		if (!sellToken) return null;

		return (
			<span>
				{showInUsd ? formatUSD(usdPerChunk) : `${formatNumber(sellTokenAmountPerChunk)} ${sellToken.symbol}`}
			</span>
		);
	},
});
