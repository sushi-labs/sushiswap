...
import { Badge, Button, Collapsible, Currency, LinkExternal, classNames } from "@sushiswap/ui";
				...
				{isHovered ? (
					...
					<>
						...
						<div className="ml-auto flex flex-col items-end text-slate-900 dark:text-[#FFF5FA]">
							...
							<span className="font-medium text-muted-foreground">{formatNumber(82.2)} SUSHI</span>
						</div>
					</>
				)}
			</div>
					...
					<div
						...
						<div className="flex w-full col-span-4 flex-col gap-4">
							...
							<div className="flex items-center gap-5">
								...
								<Button className="w-full" size="sm" onClick={() => toggleBridgeView("close")}>
									Confirm
								</Button>
							</div>
						</div>
...
const FavoriteButton = () => {
	...
	return (
		<button
			...
			type="button">
			<StarIcon width={16} height={16} className={classNames(isFavorited ? "text-yellow" : "")} />
		</button>
	);
};
