...
const resolveViewModeFromPathname = (pathname: string): TradeView => {
	...
	return TRADE_VIEWS.includes(view as TradeView) ? (view as TradeView) : "simple";
};
 ...
 * /swap || /limit || /dca ...
 */
const DerivedstateSimpleTradeProvider: FC<DerivedStateSimpleTradeProviderProps> = ({ children }) => {
	...
	const [tradeView, _setTradeView] = useState<TradeView>(resolveViewModeFromPathname(pathname));
	...
	const setTradeView = useCallback(
		(view: TradeView) => {
			...
			createQuery([], `/${pathname?.split("/")[1]}/${tradeMode}${view !== "simple" ? `/${view}` : ""}`);
		},
	...
	return (
		<DerivedStateSimpleTradeContext.Provider
			...
			}, [tradeMode, tradeView, chainId, setTradeMode, setTradeView, tradeModeChanged])}>
...
const useDerivedStateSimpleTrade = () => {
	...
	if (!context) {
		throw new Error("Hook can only be used inside Simple Trade Derived State Context");
	}
