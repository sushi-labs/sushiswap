...
import { type TimeDuration, TimeUnit, zeroAddress } from "@orbs-network/twap-sdk";
import {
	type Dispatch,
	type FC,
	type SetStateAction,
...
const DerivedStateTwapProvider: FC<DerivedStateTwapProviderProps> = ({ children, isLimitOrder }) => {
	return (
		<DerivedstateSimpleSwapProvider>
			<_DerivedStateTwapProvider isLimitOrder={isLimitOrder}>{children}</_DerivedStateTwapProvider>
		</DerivedstateSimpleSwapProvider>
	);
};

const _DerivedStateTwapProvider: FC<DerivedStateTwapProviderProps> = ({ children, isLimitOrder = false }) => {
	...
	const [isLimitPriceInverted, setIsLimitPriceInverted] = useState<boolean>(false);
	const [limitPriceString, setLimitPrice] = useState<string>("");
	const [expiry, setExpiry] = useState<TimeDuration>(TwapExpiryTimeDurations.Day);
	...
	const [marketPrice, token0PriceUSD, token1PriceUSD] = useMemo(() => {
		const [token0, token1] = [derivedStateSimpleSwap.state.token0, derivedStateSimpleSwap.state.token1];
		...
		if (token0PriceFraction && token1PriceFraction) {
			...
			const quoteRaw = token0OverToken1.multiply(parseUnits("1", token1.decimals)).quotient;

			return [
				new Price({
					baseAmount: Amount.fromRawAmount(token0, parseUnits("1", token0.decimals)),
	...
	}, [prices, derivedStateSimpleSwap.state.token0, derivedStateSimpleSwap.state.token1]);

	return (
		<DerivedStateTwapContext.Provider
			value={useMemo(() => {
				const { state, mutate, isLoading, isToken0Loading, isToken1Loading } = derivedStateSimpleSwap;

				const chainId = isTwapSupportedChainId(state.chainId0) ? state.chainId0 : ChainId.ETHEREUM;

				const sdk = TwapSDK.onNetwork(chainId);

				const duration = sdk.getOrderDuration(chunks, fillDelay, isLimitOrder ? expiry : undefined);
				...
				const amountInPerChunk =
					state.swapAmount && chunks
						? Amount.fromRawAmount(
								...
								sdk.getSrcTokenChunkAmount(state.swapAmount.quotient.toString(), chunks)
						  )
						: undefined;
				...
				const baseAmount = baseCurrency
					? Amount.fromRawAmount(baseCurrency, parseUnits("1", baseCurrency.decimals))
					: undefined;

				const quoteAmount = tryParseAmount(limitPriceString, quoteCurrency);

				const _limitPrice = baseAmount && quoteAmount ? new Price({ baseAmount, quoteAmount }) : undefined;

				const limitPrice = isLimitPriceInverted ? _limitPrice?.invert() : _limitPrice;

				const orderPrice = isLimitOrder ? limitPrice : marketPrice;

				const orderPriceOfOneToken0 = orderPrice
					? orderPrice.quote(
							Amount.fromRawAmount(orderPrice.baseCurrency, parseUnits("1", orderPrice.baseCurrency.decimals))
					  )
					: undefined;
				...
				const amountOut =
					state.token1 && destTokenAmount ? Amount.fromRawAmount(state.token1, destTokenAmount) : undefined;
				...
				const minAmountOut =
					state.token1 && destMinAmount ? Amount.fromRawAmount(state.token1, destMinAmount) : undefined;
...
const bigIntValidator = z.preprocess((v) => (typeof v === "string" ? BigInt(v) : v), z.bigint());
...
const useTwapTrade = () => {
	...
	return useMemo(() => {
		if (!swapAmount || !token0 || !token1 || !minAmountOut || !amountInPerChunk || !fillDelay || !chunks)
		...
		const { success, data: params, error } = prepareOrderArgsValidator.safeParse(sdkParams);
...
const useTwapTradeErrors = () => {
	const {
		state: { chainId, amountInPerChunk, fillDelay, token0PriceUSD, chunks, isLimitOrder },
	} = useDerivedStateTwap();

	return useMemo(() => {
		const sdk = TwapSDK.onNetwork(chainId);

		const minFillDelayError = isLimitOrder ? false : sdk.getMinFillDelayError(fillDelay).isError;
		const maxFillDelayError = isLimitOrder ? false : sdk.getMaxFillDelayError(fillDelay, chunks).isError;
	...
	}, [chainId, isLimitOrder, fillDelay, chunks, amountInPerChunk, token0PriceUSD]);
};

export { DerivedStateTwapProvider, useDerivedStateTwap, useTwapTrade, useTwapTradeErrors };
