...
import { Amount, Native, tryParseAmount } from 'sushi/currency'
import { formatUSD, shortenAddress } from 'sushi/format'
import { ZERO } from 'sushi/math'
import { Address, SendTransactionReturnType, stringify } from 'viem'
import {
  useAccount,
  usePublicClient,
  useSendTransaction,
...
export const TwapTradeTxDialog: FC<{
  children: ReactNode
}> = ({ children }) => {
  const {
    state: {
  ...
  return (
    <DialogProvider>
      ...
      <DialogConfirm
        chainId={chainId}
        status={status}
        testId="limit-submitted"
        txHash={data}
        successMessage="Limit submitted"
