...
import type { GetPools, PoolChainId, Pools } from "@sushiswap/graph-client/data-api";
import {
	Button,
	Card,
	CardHeader,
...
import type { ColumnDef, Row, SortingState, TableState } from "@tanstack/react-table";
						...
						<DropdownMenuGroup>
							<TooltipProvider>
								<Tooltip delayDuration={0}>
									<TooltipTrigger asChild={isMerklChainId(row.original.chainId)}>
										<DropdownMenuItem asChild disabled={!isMerklChainId(row.original.chainId)}>
											<Link
												onClick={(e) => e.stopPropagation()}
												shallow={true}
												className="flex items-center"
												href={`/${ChainKey[row.original.chainId]}/pool/incentivize?fromCurrency=${
													row.original.token0Address === Native.onChain(row.original.chainId).wrapped.address
														? "NATIVE"
														: row.original.token0Address
												}&toCurrency=${
													row.original.token1Address === Native.onChain(row.original.chainId).wrapped.address
														? "NATIVE"
...
export const PoolsTableV2: FC<PoolsTableV2Props> = ({ chainId, onRowClick, forcedTokenSymbols }) => {
	...
	const [data, count] = useMemo(
		() => [pools?.pages?.flatMap(({ data }) => data) ?? [], pools?.pages?.[0]?.count],
		[pools]
	);
