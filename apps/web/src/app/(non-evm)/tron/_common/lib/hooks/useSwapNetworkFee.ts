...
export const useSwapNetworkFee = ({
	...
	return useQuery({
		queryKey: ["useSwapNetworkFee", token0, token1, amountIn, route, swapType, address, minOutput],
		queryFn: async () => {
			...
			const energyCostParam =
				chainParameters.find((param: { key: string; value: string }) => param.key === "getEnergyFee")
					?.value ?? 0;
