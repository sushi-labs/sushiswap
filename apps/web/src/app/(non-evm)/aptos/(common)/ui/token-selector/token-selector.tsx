...
import { Badge, Button, Dialog, DialogContent, DialogDescription, List, TextField } from "@sushiswap/ui";
			...
			<DialogContent className="!flex flex-col justify-start min-h-[85vh]">
				...
				<List.Control className="relative flex flex-1 flex-col flex-grow gap-3 px-1 py-0.5 min-h-[128px]">
					...
					<div
						...
						{queryToken && !customTokens[`${queryToken.address}`] && !tokens?.[`${queryToken.address}`] && (
...
const CommonTokenButton = ({ token, onSelect }: { token: Token; onSelect: (token: Token) => void }) => {
	return (
		<Button
