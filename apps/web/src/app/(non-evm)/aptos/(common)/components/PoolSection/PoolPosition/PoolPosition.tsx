...
import { Card, CardContent, CardDescription, CardHeader, CardTitle, SkeletonText } from "@sushiswap/ui";
...
export const PoolPosition: FC<PoolPositionProps> = ({ row, isLoading, stakeAmount }) => {
	...
	const token0UnstakedInUsd = token0Price ? token0Price * Number(underlying0 ?? 0) : 0;
	const token1UnstakedInUsd = token1Price ? token1Price * Number(underlying1 ?? 0) : 0;
	const token0StakedInUsd = token0Price ? token0Price * Number(stakedUnderlying0 ?? 0) : 0;
	const token1StakedInUsd = token1Price ? token1Price * Number(stakedUnderlying1 ?? 0) : 0;
