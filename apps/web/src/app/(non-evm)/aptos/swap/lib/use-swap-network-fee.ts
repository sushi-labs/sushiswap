import { Account, Aptos, AptosConfig, Ed25519Account, Ed25519PublicKey } from "@aptos-labs/ts-sdk";
...
export const useSwapNetworkFee = () => {
	...
	const { bestRoutes, token0, slippageAmount, amount, token1, outputAmount } = useSimpleSwapState();
	const { account } = useWallet();

	return useQuery({
		queryKey: ["useSwapNetworkFee", bestRoutes, token0, account, slippageAmount, amount],
		queryFn: async () => {
			if (!account || !amount || bestRoutes?.length === 0 || !slippageAmount || !token0) return 0;
			...
			try {
				...
				const minOut = parseUnits(minOutput?.toString(), token1.decimals).toString();
				const amountIn = parseUnits(amount, token0.decimals).toString();

				const payload = getSwapPayload(swapContract, parseInt(amountIn), bestRoutes, parseInt(minOut));
				...
				const networkFee = Number.isNaN(_networkFee) ? 0.000014 : _networkFee / 10 ** 8;
				return networkFee;
			} catch (error) {
