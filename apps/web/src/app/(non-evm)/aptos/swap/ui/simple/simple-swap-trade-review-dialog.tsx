...
import { Button, Dots, IconButton, List, SkeletonBox, SkeletonText, classNames } from "@sushiswap/ui";
...
export const SimpleSwapTradeReviewDialog: FC = () => {
	...
	return (
		<>
			<Modal.Review modalType={ModalType.Regular} variant="transparent" tag="review-modal">
				{({ close }) => (
							...
							<div className="flex flex-col flex-grow">
								{!outputAmount || isPriceFetching ? (
									...
									<h1 className="text-lg font-semibold dark:text-slate-50">
										Buy {formatNumberWithDecimals(Number(outputAmount), token1.decimals)} {token1?.symbol}
									</h1>
								)}
									...
									<List.KeyValue
										...
										<span
											...
											{isPriceFetching ? (
												...
												<>{routes?.priceImpact ? (-routes?.priceImpact).toFixed(4) : 0}%</>
											)}
										</span>
									</List.KeyValue>
									<List.KeyValue
