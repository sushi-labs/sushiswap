...
import { SupportedNetwork, chains, networkNameToNetwork } from "~aptos/(common)/config/chains";
import { useNetwork } from "~aptos/(common)/lib/common/use-network";
import { PoolExtended, usePoolsExtended } from "./use-pools-extended";
import { AptosSDK } from "~aptos/(common)/lib/common/aptos-sdk";
...
const userPositionPoolsQueryFn = async ({ address, allPools = [], network }: UserPositionPoolsQueryFn) => {
		...
		if (allPools.length) {
			...
			const userPositionsWithApr = userPositions.map((pool) => {
				const foundPool = pools?.find(
					(_pool) =>
						_pool?.token0Address === pool?.token0?.address && _pool?.token1Address === pool?.token1?.address
				);
