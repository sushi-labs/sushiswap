...
export const PoolMyPositionTVLCell: FC<Row<PoolExtendedWithAprVolume> & { isSize: boolean }> = ({
	...
	const { data: liquidityBalance, isLoading: isLoadingLpBalance } = useTokenBalance({
	...
	const userPositionTvl =
		token0 && token1
			? (token0Price ?? 0) * Number(formatNumberWithDecimals(Number(currencyABalance), token0.decimals)) +
			  (token1Price ?? 0) * Number(formatNumberWithDecimals(Number(currencyBBalance), token1.decimals))
			: 0;
	...
	const isLoading = isLoadingLpBalance || token0Price === undefined || token1Price === undefined;
