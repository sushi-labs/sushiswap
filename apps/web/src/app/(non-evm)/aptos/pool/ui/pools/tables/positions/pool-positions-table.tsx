...
import { PoolExtendedWithAprVolume, useUserPositionPools } from "~aptos/pool/lib/use-user-position-pools";
import { usePoolFilters } from "../../filters/pool-filters-provider";
import { MYPOSITION_APR_COLUMN, MYPOSITION_TVL_COLUMN, NAME_COLUMN, TVL_COLUMN } from "./columns";

const COLUMNS = [NAME_COLUMN, TVL_COLUMN, MYPOSITION_TVL_COLUMN, MYPOSITION_APR_COLUMN] satisfies ColumnDef<
...
export const PositionsTable = () => {
	...
	const { data: pools, isLoading, isPending } = useUserPositionPools(account?.address as string, true);
		...
		<Card>
			<CardHeader>
				<CardTitle>
					Pools{" "}
					{filtered.length ? (
						<span className="text-gray-400 dark:text-slate-500">({filtered.length})</span>
					) : null}
				</CardTitle>
			</CardHeader>
			<DataTable
