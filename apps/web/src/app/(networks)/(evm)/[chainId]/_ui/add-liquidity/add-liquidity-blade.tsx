...
export const AddLiquidityBlade = ({
  ...
  const appendToken = useCallback((token?: EvmCurrency) => {
    if (!token) return
    setTokens((prev) => {
  ...
  const replaceTokenAt = useCallback((index: number, token?: EvmCurrency) => {
    if (!token) return
    setTokens((prev) => {
  ...
  const { data: p0, isLoading: l0 } = usePrice({
    chainId: t0?.chainId,
    address: t0?.wrap()?.address,
    enabled: !!t0,
  })
  const { data: p1, isLoading: l1 } = usePrice({
    chainId: t1?.chainId,
    address: t1?.wrap()?.address,
    enabled: !!t1,
  })
  const { data: p2, isLoading: l2 } = usePrice({
    chainId: t2?.chainId,
    address: t2?.wrap()?.address,
    enabled: !!t2,
  })
  const { data: p3, isLoading: l3 } = usePrice({
    chainId: t3?.chainId,
    address: t3?.wrap()?.address,
    enabled: !!t3,
  })
  const { data: p4, isLoading: l4 } = usePrice({
    chainId: t4?.chainId,
    address: t4?.wrap()?.address,
    enabled: !!t4,
  })

  const priceFor = useCallback(
    (token?: EvmCurrency): number | undefined => {
...
export type AddLiquidityWidgetProps = {
  ...
  appendToken?: (token?: EvmCurrency) => void
...
const AddLiquidityWidget: FC<AddLiquidityWidgetProps> = ({
  ...
  const parsedInput0 = useMemo(() => {
    ...
    return Amount.tryFromHuman(token0, input0) || Amount.tryFromHuman(token0, 0)
  }, [token0, input0])

  const parsedInput1 = useMemo(() => {
    if (!token1) return undefined
    return Amount.tryFromHuman(token1, input1) || Amount.tryFromHuman(token1, 0)
  }, [token1, input1])
  ...
  const renderRow = (t: EvmCurrency, i: number) => {
    ...
    const onSelect = hideTokenSelectors
      ? undefined
      : (next?: EvmCurrency) => replaceTokenAt(i, next)
