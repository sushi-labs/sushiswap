...
import { SparklineCell } from '../explore/tokens/_ui/sparkline-cell'
  ...
  cell: (props) => {
    const [token0, token1] = useMemo(() => {
      const [symbol0 = '', symbol1 = ''] =
        props.row.original.name?.split(' / ') ?? []

      const chainId = props.row.original.chainId as EvmChainId

      return [
        new EvmToken({
          name: symbol0,
          chainId,
          address: props.row.original.token0Address as `0x${string}`,
          symbol: symbol0,
          decimals: 0,
        }),
        new EvmToken({
          name: symbol1,
          chainId,
          address: props.row.original.token1Address as `0x${string}`,
          symbol: symbol1,
          decimals: 0,
        }),
      ]
...
export const ACTION_COLUMN: ColumnDef<MultiChainPool, unknown> = {
  ...
  cell: (props) => {
    ...
    const [token0, token1] = useMemo(() => {
      const [token0, token1] = [
        _token0 ? unwrapEvmToken(_token0) : undefined,
        _token1 ? unwrapEvmToken(_token1) : undefined,
      ]
