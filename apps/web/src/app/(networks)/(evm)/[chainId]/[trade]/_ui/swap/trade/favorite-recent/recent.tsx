...
import { type RecentSwap, TokenListV2ChainIds, isTokenListV2ChainId } from "@sushiswap/graph-client/data-api";
...
import { captializeFirstLetter, getChangeSign, getTextColor } from "src/lib/helpers";
	...
	if (recentSwaps?.length === 0 && !isLoading && !isError) {
		return (
			<p className="my-8 text-sm italic text-center text-muted-foreground dark:text-pink-200">
				...
				{selectedNetwork ? ` on ${captializeFirstLetter(getNetworkKey(selectedNetwork))}` : ""}.
			</p>
		);
	}
	...
	return (
		<div className="grid grid-cols-3 col-span-3 gap-0">
			<div className="sticky top-0 z-[19  ] grid grid-cols-5 col-span-5 text-xs bg-white md:bg-slate-50 dark:bg-slate-900 md:dark:bg-slate-800 text-slate-700 dark:text-pink-100">
				...
				<div className="w-full font-medium text-left whitespace-nowrap">Amount Traded</div>
				<div className="w-full col-span-1 pr-2 font-medium text-right">
					<Popover>
						<PopoverTrigger asChild>
							<span className="underline cursor-pointer decoration-dotted">PnL</span>
						</PopoverTrigger>
						...
						<PopoverContent
							...
							<div className="max-w-[250px] px-1 py-2 text-black dark:text-pink-100 text-sm">
								Profit or loss calculated as the difference in USD value of the asset on the day it was bought
								and the day it was sold.
							</div>
						</PopoverContent>
					</Popover>
				</div>
			...
			<div className="grid w-full grid-cols-5 col-span-5 gap-2">
				{isLoading
					...
					: recentSwaps?.map((recentSwap, idx) => (
							<RecentItem onClose={onClose} key={`${idx}_${recentSwap?.time}`} recentSwap={recentSwap} />
					  ))}
			</div>
		</div>
	);
...
const RecentItem = ({ recentSwap, onClose }: { recentSwap: RecentSwap; onClose?: () => void }) => {
		...
		<>
			<div
				...
				<div className="flex items-center w-full col-span-3 gap-2">
					<Currency.IconList iconWidth={24} iconHeight={24} className="border-none">
						<Currency.Icon
							disableLink
							currency={
				...
				{isHovered && isMd ? (
					...
					<>
						...
						<div className="w-full col-span-1 ml-auto text-right">
							<span className={classNames("font-medium", getTextColor(recentSwap?.totalPnl))}>
...
const ActionButtons = ({ recentSwap, onClose }: { recentSwap: RecentSwap; onClose?: () => void }) => {
