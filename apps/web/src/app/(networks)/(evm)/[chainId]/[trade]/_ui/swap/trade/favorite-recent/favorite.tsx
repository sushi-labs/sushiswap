...
export const Favorite = () => {
	...
	const onSelect = useCallback(
		(_token: EvmCurrency) => {
			const currencyId: PinnedTokenId = `${_token?.id}:${_token?.symbol}` as PinnedTokenId;
	...
	return (
		<div>
			{isLoading ? (
				...
				<div className="text-center text-sm text-red mt-4">An error occurred loading favorites</div>
			) : favorites?.length !== 0 ? (
				<table className="w-full">
					<thead className="sticky top-0 z-[19] bg-slate-50 dark:bg-slate-900 md:dark:bg-slate-800">
						<tr className="text-xs text-slate-700 dark:text-pink-100">
							...
							<th className="hidden font-medium text-left md:table-cell">Price</th>
							<th className="hidden font-medium text-left md:table-cell">24h%</th>
							<th className="table-cell font-medium text-left md:hidden">Price/24%</th>
			...
			<div className="flex flex-col items-center justify-center w-full gap-4 mt-6">
				<TokenSelectorV2
					selected={undefined}
					selectedNetwork={
						selectedNetwork ? (selectedNetwork as EvmChainId) : chainId ? (chainId as EvmChainId) : undefined
					}
					onSelect={onSelect}
					includeNative={true}
				...
				{!isLoading && !isError && favorites?.length === 0 ? (
					<p className="text-sm italic text-muted-foreground dark:text-pink-200">
						...
						{selectedNetwork ? `on ${getNetworkKey(selectedNetwork as EvmChainId)}` : ""}.
					</p>
				) : null}
			</div>
		</div>
	...
	return (
		<tr
			...
			<td className="hidden md:table-cell">
				<span className="text-slate-900 dark:text-pink-100">{formatUSD(token.priceUSD)}</span>
			</td>
			<td className="hidden md:table-cell">
				<span className={classNames("font-medium", getTextColor(token.priceChange1d))}>
