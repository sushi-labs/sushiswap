...
export const PoolHeader = ({ poolId }: { poolId: string }) => {
	...
	useEffect(() => {
		if (pool?.token0) {
			const _token0 = baseTokens?.find((token) => token.tokenAddress === pool.token0.address);
			setToken0({
				tokenAddress: pool?.token0?.address,
				tokenSymbol: _token0?.tokenSymbol ?? pool?.token0?.name.slice(0, 4)?.toUpperCase(),
		...
		if (pool?.token1) {
			const _token1 = baseTokens?.find((token) => token.tokenAddress === pool.token1.address);
			setToken1({
				tokenAddress: pool?.token1?.address,
				tokenSymbol: _token1?.tokenSymbol ?? pool?.token1?.name.slice(0, 4)?.toUpperCase(),
	...
	}, [pool?.token0, pool?.token1, setToken0, setToken1, baseTokens, decimals0, decimals1]);
              ...
              V2
				...
				<div className="flex flex-wrap items-center gap-y-5 gap-x-[32px] text-secondary-foreground mb-8 mt-1.5">
					...
					{isLoading ? (
						...
						<>
							<div className="flex items-center gap-1.5">
								<span className="font-semibold tracking-tighter">{token0?.tokenSymbol}</span>
								<LinkExternal target="_blank" href={getChainwebAddressLink(token0?.tokenAddress ?? "")}>
									<Button
										asChild
										variant="link"
										size="sm"
							...
							<div className="flex items-center gap-1.5">
								<span className="font-semibold tracking-tighter">{token1?.tokenSymbol}</span>
								<LinkExternal target="_blank" href={getChainwebAddressLink(token1?.tokenAddress ?? "")}>
									<Button
										asChild
										variant="link"
										size="sm"
