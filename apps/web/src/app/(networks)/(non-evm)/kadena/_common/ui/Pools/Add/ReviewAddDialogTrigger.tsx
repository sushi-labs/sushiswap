...
export const ReviewAddDialogTrigger = (props: ButtonProps) => {
	const { token0, token1, isTxnPending, amountInToken0, amountInToken1, poolId } = usePoolState();
	...
	const hasInsufficientGas = useMemo(() => {
		...
		const kdaBalance = parseFloat(balanceMap?.["coin"] ?? "0");
	...
	const token0Balance = token0 && balanceMap ? parseFloat(balanceMap[token0?.address]) : 0;
	const token1Balance = token1 && balanceMap ? parseFloat(balanceMap[token1?.address]) : 0;
