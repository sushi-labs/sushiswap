...
export const WalletListView = ({ isFullWidth = false }: { isFullWidth?: boolean }) => {
	const { adapters, handleConnect } = useKadena();
	return (
		<List
			className={`flex flex-col gap-1 !p-0 ${
				isFullWidth ? "[width:calc(var(--radix-popover-trigger-width)_-_10px)]" : ""
			}`}>
			<List.Control className="bg-gray-100">
				{adapters.map((adapter) => (
					<List.MenuItem
						...
						onClick={() => {
							adapter.detected ? handleConnect(adapter.name) : window.open(adapter.installUrl, "_blank");
						}}
