		...
		<Card>
			...
			<CardContent>
				<div className="grid grid-cols-1 gap-3">
					<Item label="Liquidity" value={formatUSD(pool?.tvlUsd ?? 0)} change={pool?.tvlChange24h ?? 0} />
					<Item
						label="Volume (24h)"
					...
					<Item
						...
						value={formatNumber(pool?.transactionCount24h ?? 0).replace(".00", "")}
...
const Item = ({ label, value, change }: { label: string; value: number | string; change: number }) => {
	return (
		<div>
