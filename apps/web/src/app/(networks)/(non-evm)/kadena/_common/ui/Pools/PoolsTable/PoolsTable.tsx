...
import { Card, CardHeader, CardTitle, DataTable, SkeletonText } from "@sushiswap/ui";
...
export const PoolsTable = () => {
	...
	const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isLoading } = useAllPools({
		first: 10,
	...
	useEffect(() => {
		if (data?.pools && end > data?.pools?.length && hasNextPage && !isFetchingNextPage) {
			fetchNextPage();
		}
