...
export const RemoveButton = (props: ButtonProps) => {
	...
	const removeLiquidity = async () => {
		...
		try {
			...
			if (preflightResult.result.status === "failure") {
				throw new Error(preflightResult.result.error?.message || "Preflight failed");
			}
	...
	const onSuccess = async () => {
		...
		queryClient.invalidateQueries({
			queryKey: ["kadena-lp-balance", address, token0?.address, token1?.address],
		});
		queryClient.invalidateQueries({
	...
	return (
		<Button onClick={removeLiquidity} disabled={percentage === 0 || isTxnPending} {...props}>
			{buttonText}
		</Button>
	);
};
