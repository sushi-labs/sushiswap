...
export const MinimumReceive = () => {
	...
	const [slippageTolerance] = useSlippageTolerance(SlippageToleranceStorageKey.RemoveLiquidity);
	...
	useEffect(() => {
		if (minAmountToken0 && token0) {
			setMinAmountToken0(new Decimal(formatToMaxDecimals(minAmountToken0, token0?.tokenDecimals)).toNumber());
		}
	...
	useEffect(() => {
		if (minAmountToken1 && token1) {
			setMinAmountToken1(new Decimal(formatToMaxDecimals(minAmountToken1, token1?.tokenDecimals)).toNumber());
		}
