import { Button, type ButtonProps, Chip, Popover, PopoverContent, PopoverTrigger } from "@sushiswap/ui";
	...
	return (
		<Popover>
			<PopoverTrigger className="relative w-full">
				<Button loading={isConnecting} disabled={isConnecting} asChild {...props}>
					{isConnected && activeAccount?.accountName ? (
						<>
							<JazzIcon diameter={20} address={activeAccount?.accountName} />
							<span className="hidden sm:block">{truncateText(activeAccount?.accountName)}</span>
						</>
					) : (
