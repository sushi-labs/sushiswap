...
export const ReviewSwapDialogTrigger = () => {
	...
	const { amountIn, isTxnPending, token0, token1, priceImpactPercentage, route, amountOut } = useSwapState();

	const { activeAccount } = useKadena();

	const { data: tokenBalances, isLoading: isLoadingTokenBalance } = useTokenBalances({
		account: activeAccount?.accountName ?? "",
		tokenAddresses: token0 && token1 ? [token0?.tokenAddress, token1?.tokenAddress] : [],
	});
	const balanceMap = tokenBalances?.balanceMap ?? undefined;

	const token0Balance = token0 && balanceMap ? balanceMap[token0?.tokenAddress] : 0;
	...
	const insufficientLiquidity =
		priceImpactPercentage >= 100 || !data?.poolData?.reserve0 || !data?.poolData?.reserve1;
	...
	const buttonText = useMemo(() => {
		...
		return "Swap";
	}, [amountIn, hasInsufficientToken0Balance, noRoutes, insufficientLiquidity, isTxnPending]);
