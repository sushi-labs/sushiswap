...
export const buildGetPoolExists = (token0: string, token1: string, chainId: number, networkId: string) => {
...
export const buildGetPoolAddress = (token0: string, token1: string, chainId: number, networkId: string) => {
...
export const buildAddLiquidityTxn = ({
	...
	if (!_poolAddress) {
		...
		const tx = Pact.builder
			...
			.addSigner(pubKey, (signFor) => [signFor("coin.GAS"), signFor(`${KADENA_CONTRACT}.GOVERNANCE`)])
			.setNetworkId(networkId)
			.createTransaction();
		return tx;
	}
	const tx = Pact.builder
		...
		.addSigner(pubKey, (signFor) => [
			signFor("coin.GAS"),
			signFor(`${token0Address}.TRANSFER`, `${signerAddress}`, `${poolAddress}`, {
				decimal: amountInToken0.toString(),
			}),
			signFor(`${token1Address}.TRANSFER`, `${signerAddress}`, `${poolAddress}`, {
