...
export const AmountIn = () => {
	...
	const { setToken0, setAmountIn, setAmountOut, setRoute, setPriceImpactPercentage } = useSwapDispatch();
	...
	const calculatedPriceImpact: number = useMemo(() => {
		if (!data?.poolData?.reserve0 || !data?.poolData?.reserve1 || !debouncedAmountIn) {
			return 0;
		}
		...
		const priceImpact = idealAmountOut.sub(amountOut).div(idealAmountOut).mul(100).toNumber();
