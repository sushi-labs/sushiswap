...
import type { KadenaToken, TokenWithBalance } from "~kadena/_common/types/token-type";
		...
		({ index, style }: { index: number; style: CSSProperties }) => {
			return (
				<TokenButton
					...
					isSelected={sortedTokens?.[index]?.tokenAddress === selected?.tokenAddress}
				/>
			);
		},
			...
			<Button
				...
				<div className="flex items-center w-full gap-3 ">
						...
						<Badge
							...
							badgeContent={
								<div className="bg-white rounded-full dark:bg-slate-800">
									<CheckCircleIcon width={14} height={14} className="rounded-full text-blue" />
								</div>
							}>
					...
					<div className="flex flex-col items-start">
						...
						<p className="text-xs text-gray-400 dark:text-slate-500">{token.tokenName}</p>
					</div>
				</div>
				{+(token as TokenWithBalance)?.balance > 0 ? (
					<div className="flex flex-col max-w-[140px]">
						<span
							...
							{formatNumberWithMaxDecimals((token as TokenWithBalance)?.balance ?? "0")}
						</span>
					</div>
				) : null}
			</Button>
