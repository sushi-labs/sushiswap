...
import { APR_COLUMN, POSITION_NAME_COLUMN, VALUE_COLUMN } from "./PositionColumns";
...
export const PositionsTable = ({ hideNewPositionButton }: PositionsTableProps) => {
	...
	const { data, isLoading, fetchNextPage, hasNextPage, isFetchingNextPage } = useMyPositions();
	...
	const filteredPositions = useMemo(() => {
		...
		const queries = tokenSymbols.map((symbol) => symbol.toLowerCase().replaceAll(" ", ""));
		...
		return positions.filter((pool) => {
			...
			return poolValues.some((value) => queries.some((query) => value.toLowerCase().includes(query)));
		});
	...
	return (
		<Card>
			<CardHeader>
				<CardTitle>
					<div className="flex flex-wrap items-center justify-between gap-4">
						<span>
							My Positions{" "}
							<span className="text-gray-400 dark:text-slate-500">({paginatedFilteredData?.length})</span>
						</span>
			...
			<DataTable
				...
				linkFormatter={(data: WalletPosition) => `/kadena/pool/${encodeURIComponent(data.pairId)}/add`}
				externalLink={false}
				pagination={true}
				state={{
