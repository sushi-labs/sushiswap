...
const KadenaWalletContext = createContext<WalletContextType | undefined>(undefined);

type KadenaWalletProviderProps = {
	children: ReactNode;
};

export const KadenaWalletProvider = ({ children }: KadenaWalletProviderProps) => {
	...
	const walletAdapters = client.getProviders().map((adapter) => ({
		...
		installUrl: ADAPTER_INSTALL_URLS[adapter.name as keyof typeof ADAPTER_INSTALL_URLS] || "",
	}));
	...
	const contextValue = useMemo(
		...
		[currentWallet, activeAccount, handleConnect, handleDisconnect, walletAdapters, isConnected, isConnecting]
	);

	return <KadenaWalletContext.Provider value={contextValue}>{children}</KadenaWalletContext.Provider>;
};

export const useKadena = (): WalletContextType => {
	const context = useContext(KadenaWalletContext);
	if (!context) throw new Error("useKadena must be used within a KadenaWalletProvider");
	return context;
};
