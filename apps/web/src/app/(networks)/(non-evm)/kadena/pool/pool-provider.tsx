...
import { type FC, createContext, useContext, useEffect, useMemo, useReducer } from "react";
import { PoolChartPeriod } from "src/ui/pool/PoolChartPeriods";
import { DEFAULT_TOKEN_LIST, KADENA, STABLE_TOKENS } from "~kadena/_common/constants/token-list";
...
type Dispatch = {
	...
	setPoolByIdChartTimeFrame(poolByIdChartTimeFrame: PoolByIdChartTimeFrame): void;
...
function poolReducer(_state: State, action: Action) {
	switch (action.type) {
		case "setToken0": {
			...
			if (
				(_state?.token1?.tokenSymbol === "KDA" && action.value.tokenSymbol === "WKDA") ||
				(_state?.token1?.tokenSymbol === "WKDA" && action.value.tokenSymbol === "KDA")
			) {
				return {
					..._state,
		...
		case "setToken1": {
			...
			if (
				(_state?.token0?.tokenSymbol === "KDA" && action.value.tokenSymbol === "WKDA") ||
				(_state?.token0?.tokenSymbol === "WKDA" && action.value.tokenSymbol === "KDA")
			) {
				return {
					..._state,
					token0: DEFAULT_TOKEN_LIST[0],
...
const PoolProvider: FC<PoolProviderProps> = ({ children }) => {
	...
	const dispatchWithAction = useMemo(
		() => ({
			...
			setIsLoadingPool: (value: boolean) => dispatch({ type: "setIsLoadingPool", value }),
			setTotalSupplyLP: (value: number) => dispatch({ type: "setTotalSupplyLP", value }),
		}),
		[]
	);
	...
	useEffect(() => {
		if (data) {
			...
			dispatchWithAction.setRateOfToken0ToToken1(data?.poolData?.rateOfToken0ToToken1);
			dispatchWithAction.setRateOfToken1ToToken0(data?.poolData?.rateOfToken1ToToken0);
