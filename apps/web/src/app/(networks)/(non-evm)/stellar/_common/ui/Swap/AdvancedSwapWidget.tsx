...
import { useAddLiquidity, useSwapWithRouting, useGetQuote, usePoolsBetween } from '../../lib/hooks/swap'
...
export function AdvancedSwapWidget({ userAddress, tokens }: AdvancedSwapWidgetProps) {
  ...
  const amountInBigInt = selectedTokenIn && amountIn 
    ? BigInt(Math.floor(parseFloat(amountIn) * Math.pow(10, selectedTokenIn.decimals)))
    : 0n

  // Get quote for the swap
  const { data: quote, isLoading: isQuoteLoading, error: quoteError } = useGetQuote({
  ...
  const handleAddLiquidity = async () => {
    ...
    try {
      await addLiquidityMutation.mutateAsync({
        ...
        token1Amount: quote ? (Number(quote.amountOut) / Math.pow(10, selectedTokenOut.decimals)).toString() : '0',
        tickLower: -60000,
        tickUpper: 60000
      })
    } catch (error) {
  ...
  return (
    <div className="w-full max-w-md mx-auto p-6 bg-white rounded-lg shadow-lg">
      <h2 className="text-2xl font-bold mb-6 text-center">Advanced Swap Widget</h2>
        ...
        <div className="mt-6 p-4 bg-gray-50 rounded-md">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-mono">
                {selectedTokenOut ? 
                  (Number(quote.amountOut) / Math.pow(10, selectedTokenOut.decimals)).toFixed(6) 
                  : '0'
