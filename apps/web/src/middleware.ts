...
export async function middleware(req: NextRequest) {
	...
	if (pathname === "portal" || pathname.startsWith("/portal/")) {
		const portalMiddleware = (await import("./app/portal/middleware")).portalMiddleware;
	...
	if (
		...
		const path = ["/explore", "/pools"].includes(pathname) ? "explore/pools" : pathname.slice(1);
		...
		if (cookie) {
			...
			if (isSushiSwapChainId(chainId)) {
				return NextResponse.redirect(new URL(`/${ChainKey[chainId]}/${path}`, req.url));
			}
		}
	...
	if (networkNameMatch?.length) {
		...
		if (page === "swap") {
			if (search !== "" && searchParams.has("token0") && searchParams.has("token1")) {
