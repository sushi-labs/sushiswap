...
export function useCreateStore(initialState = {}) {
  ...
  useLayoutEffect(() => {
    const subscribe = async () => {
      ...
      if (!priorityConnectorWithStore) return
      priorityConnectorWithStore[2].subscribe(() => {
        const { accounts, chainId, activating, error } = priorityConnectorWithStore[2].getState()

        store.getState().setWeb3({ account: accounts?.[0], chainId, activating, error })
      })
    }
