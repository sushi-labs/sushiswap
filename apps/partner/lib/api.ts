...
export const getExchangeTokenKPI = async (
  id: string,
  chainId: SushiSwapChainId
): Promise<TokenKPI | undefined> => {
  const [subgraphHost, subgraphName] = [
    GRAPH_HOST[chainId],
    EXCHANGE_SUBGRAPH_NAME[chainId],
  ];
  ...
  const { Exchange_token: token, Exchange_bundle: bundle } =
...
export const getTridentTokenKPI = async (
  id: string,
  chainId: TridentChainId
): Promise<TokenKPI | undefined> => {
  const [subgraphHost, subgraphName] = [
    GRAPH_HOST[chainId],
    TRIDENT_SUBGRAPH_NAME[chainId],
  ];
  ...
  const { Trident_token: token, Trident_bundle: bundle } =
    await sdk.TridentToken({
      id: id.toLowerCase(),
  ...
  return {
    ...
    liquidity: Number(token.liquidity),
    liquidityUSD:
      token.liquidity * token.price.derivedNative * bundle.nativePrice,
    volumeUSD: Number(token.volumeUSD),
  };
};
