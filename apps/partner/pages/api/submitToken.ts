...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (!process.env.LIST_PR_WEBHOOK_URL)
    throw new Error("LIST_PR_WEBHOOK_URL undefined");
  ...
  const exchangeData = await getTokenKPI(
    tokenAddress,
    chainId as SushiSwapChainId | TridentChainId
  );
