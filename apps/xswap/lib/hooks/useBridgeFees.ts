...
export const useBridgeFees = ({
  ...
  const adjusted = useMemo(() => {
    ...
    return localDecimals > sharedDecimals
      ? amount.asFraction.divide(
          JSBI.exponentiate(
            JSBI.BigInt(10),
            JSBI.BigInt(localDecimals - sharedDecimals)
          )
        )
      : amount.asFraction.multiply(
          JSBI.exponentiate(
            JSBI.BigInt(10),
            JSBI.BigInt(sharedDecimals - localDecimals)
          )
        );
