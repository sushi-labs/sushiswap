...
interface ConfirmationComponentController {
  ...
  children:
    | ReactElement
    | ReactElement[]
    | ((payload: RenderProps) => ReactElement);
  className?: string;
  onClose?(): void;
}

export const ConfirmationComponentController: FC<
  ConfirmationComponentController
> = ({ variant, trigger, children, className, onClose }) => {
          ...
          <SlideIn.FromBottom show={open} onClose={handleClose}>
            <Overlay.Content
              className={classNames(
                className,
                "flex flex-col flex-grow !bg-slate-800 !pb-0 px-4"
              )}
            >
              <Overlay.Header
