          ...
          <span className="text-sm font-semibold text-gray-700 text-right dark:text-slate-400">
            {loading ? (
              ...
              <Modal.Trigger tag={'trade-state-routes'}>
                {({ open }) => (
                  <button onClick={open} type="button" className="text-sm text-blue font-semibold">
                    View
                  </button>
                )}
              </Modal.Trigger>
        ...
        {account?.address && (
          <div className="flex justify-between items-center border-t border-gray-200 dark:border-slate-200/5 mt-2 pt-2">
            ...
            <span className="font-semibold text-gray-700 text-right dark:text-slate-400">
              <a
                target="_blank"
                href={`https://explorer.aptoslabs.com/account/${account?.address}?network=${networkNameToNetwork(
                  network
                )}`}
