...
const LINKS = ({ pair }: { pair: Pair }): BreadcrumbLink[] => [
  {
    href: `/${pair.id}`,
    label: `${pair.name} - ${
      GET_POOL_TYPE_MAP[pair.type as keyof typeof GET_POOL_TYPE_MAP]
    } - ${formatPercent(pair.swapFee / 10000)}`,
  },
  {
    href: `/${pair.id}/remove`,
...
const Remove: FC<InferGetStaticPropsType<typeof getStaticProps>> = ({
  fallback,
}) => {
  return (
        ...
        <Layout breadcrumbs={LINKS(data)}>
          <div className="grid grid-cols-1 sm:grid-cols-[340px_auto] md:grid-cols-[auto_396px_264px] gap-10">
            ...
            <div className="flex flex-col order-3 gap-3 pb-40 sm:order-2">
              ...
              {pair.source === "TRIDENT" ? (
                <RemoveSectionTrident pair={pair} />
              ) : (
