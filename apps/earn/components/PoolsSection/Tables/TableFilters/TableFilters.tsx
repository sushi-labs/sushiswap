...
export const TableFilters: FC<{ showAllFilters?: boolean }> = ({
  ...
  const poolTypesValue =
    Object.keys(AVAILABLE_POOL_TYPE_MAP).length === selectedPoolTypes.length
      ? []
      : selectedPoolTypes;
          ...
          <Select
            ...
            onChange={(values: string[]) =>
              setFilters({
                selectedPoolTypes:
                  values.length === 0
                    ? Object.keys(AVAILABLE_POOL_TYPE_MAP)
                    : values,
              })
            }
            button={
