...
export const SushiBarSectionDesktop: FC = () => {
  ...
  const amount = useMemo(
    () => tryParseAmount(value, stake ? SUSHI_TOKEN : XSUSHI_TOKEN),
    [stake, value]
  );
    ...
    <section className="hidden md:flex">
      <div className="flex flex-col w-full gap-6">
        <h4 className="font-semibold text-slate-50">
          Earn trading fees from all pools on Sushi!
        </h4>
        <div className="p-5 flex flex-col rounded-2xl bg-white bg-opacity-[0.02]">
          <div className="flex flex-col lg:flex-row">
            <div className="flex flex-col justify-center px-5">
              <h4 className="mb-1 font-semibold text-slate-50 whitespace-nowrap">
                Sushi Bar
              </h4>
            ...
            <div className="flex items-start flex-grow gap-5 p-5">
              <div
                className={classNames(stake ? "order-1 flex-grow" : "order-3")}
              >
                <SushiBarInput
                  ...
                  value={
                    stake
                      ? value
                      : (
                          Number(value) * Number(stats?.sushiXsushiRatio)
                        ).toFixed(6)
                  }
                  ...
                  components={
                    <Approve.Components>
                      <Approve.Token
                        ...
                        amount={amount}
                        address={
                          getSushiBarContractConfig(ChainId.ETHEREUM).address
                        }
                      />
                    </Approve.Components>
                  }
