...
export const TransactionProgressDestination: FC<
  TransactionProgressDestination
> = ({ isPrevError, isPrevSuccess, onClose }) => {
  const { address } = useAccount();
  const { id, dstToken, dstChainId, amount, srcToken, timestamp } =
    useBridgeState();
  ...
  >();
  const [, { createSuccessNotification, createFailedNotification }] =
  ...
  return (
    <TransactionProgressStep
      link={
        dstTxState
          ? chains[dstAmountOut.currency.chainId].getTxUrl(dstTxState.txHash)
          : undefined
      }
      lastStep={true}
      status={
      ...
      header={
        ...
        (dstToken && dstAmountOut.currency.wrapped.equals(dstToken)) ? (
