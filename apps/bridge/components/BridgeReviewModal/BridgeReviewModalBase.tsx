...
export const BridgeReviewModalBase: FC<BridgeReviewModalBase> = ({
  open,
  setOpen,
  children,
}) => {
  const { amount, srcToken, dstToken, srcChainId, dstChainId, sourceTx } =
    useBridgeState();
  ...
  const srcTokenPrice = srcToken
    ? srcPrices?.[srcToken.wrapped.address]
    : undefined;
  const dstTokenPrice = dstToken
    ? dstPrices?.[dstToken.wrapped.address]
    : undefined;

  const [inputUsd, outputUsd, usdPctChange] = useMemo(() => {
    const inputUSD =
      amount && srcTokenPrice
        ? amount.multiply(srcTokenPrice.asFraction)
        : undefined;
    const outputUSD =
                  ...
                  <div className="flex justify-between gap-2">
                    ...
                    {dstAmountOut && (
                      ...
                      >
                        ...
                        />
                        <span>
                          {chains[dstAmountOut.currency.chainId].name}
                        </span>
                      </Typography>
                    )}
                  </div>
