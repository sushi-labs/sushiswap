...
const DerivedDataContext = createContext<BridgeStateDerived>(
  {} as BridgeStateDerived
);
...
export const BridgeStateProvider: FC<_BridgeStateProviderProps> = ({
  initialState,
  children,
}) => {
  ...
  const api = useMemo(() => {
    const setSrcChainId = (chainId: StargateChainId) =>
      dispatch({ type: "setSrcChainId", chainId });
    const setDstChainId = (chainId: StargateChainId) =>
      dispatch({ type: "setDstChainId", chainId });
    const setSrcToken = (currency: Currency) =>
      dispatch({ type: "setSrcToken", currency });
    const setDstToken = (currency: Currency) =>
      dispatch({ type: "setDstToken", currency });
    const setSrcTypedAmount = (srcTypedAmount: string) =>
      dispatch({ type: "setSrcTypedAmount", srcTypedAmount });
    const switchChainIds = () => dispatch({ type: "switchChainIds" });
    const setSourceTx = (tx: SendTransactionResult) =>
      dispatch({ type: "setSourceTx", tx });
    const setSignature = (sig: Signature) =>
      dispatch({ type: "setSignature", sig });
    const setTimestamp = (ts: number) => dispatch({ type: "setTimestamp", ts });
    const setGasFee = (gasFee: Amount<Type>) =>
      dispatch({ type: "setGasFee", gasFee });
    ...
    <APIContext.Provider value={api}>
      <DataContext.Provider value={state}>
        <DerivedDataContext.Provider
          value={useMemo(() => bridgeOutput, [bridgeOutput])}
        >
          {children}
        </DerivedDataContext.Provider>
...
export const useBridgeStateActions = () => {
  ...
  if (!context) {
    throw new Error(
      "Hook can only be used inside Bridge State Actions Context"
    );
  }
