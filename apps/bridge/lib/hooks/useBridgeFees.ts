...
export const useBridgeFees = ({
  ...
  const { data: getFeesResults, isLoading: isFeeResultLoading } =
    useContractRead({
      ...
      args: amount
        ? [
            BigNumber.from(
              STARGATE_POOL_ID[srcChainId][(srcToken as Type).wrapped.address]
            ),
            BigNumber.from(
              STARGATE_POOL_ID[dstChainId][(dstToken as Type).wrapped.address]
            ),
            STARGATE_CHAIN_ID[dstChainId],
