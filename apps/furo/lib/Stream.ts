...
export class Stream extends Furo {
  ...
  public constructor({
    ...
    this.withdrawnShares = Amount.fromRawAmount(
      this.token,
      JSBI.subtract(
        JSBI.add(this.initialShares.quotient, this.extendedShares.quotient),
  ...
  public override get balance(): Amount<Token> {
    ...
    if (this.extendedShares.equalTo(0)) {
      const duration = JSBI.subtract(
        ...
        JSBI.BigInt(this.startTime.getTime())
      );
      const passed = JSBI.subtract(
        JSBI.BigInt(Date.now()),
        JSBI.BigInt(this.startTime.getTime())
      );
      const streamedShares = JSBI.divide(
        JSBI.multiply(this.initialShares.quotient, passed),
        duration
      );
      ...
      const balance = Amount.fromRawAmount(
        this.token,
        JSBI.add(this.withdrawnAmount.quotient, pendingAmount.quotient)
      );
      ...
      const pendingAmount = Amount.fromShare(
        this.token,
        JSBI.subtract(
          streamedSharesAfterExtension,
          withdrawnSharesAfterExtension
        ),
        this.rebase
      );
  ...
  public get streamedPercentage(): Percent | undefined {
    ...
    if (this.extendedShares.equalTo(0)) {
      ...
      const totalShares = JSBI.add(
        this.initialShares.quotient,
        this.extendedShares.quotient
      );
      const streamedBeforeExtension = JSBI.subtract(
        totalShares,
        this.initialSharesExtended.quotient
      );
      ...
      const totalStreamed = JSBI.add(
        streamedSharesAfterExtension,
        streamedBeforeExtension
      );
  ...
  public get withdrawnPercentage(): Percent {
    ...
    return new Percent(
      this._withdrawnAmount.quotient,
      this.totalAmount.quotient
    );
  }
}
