...
export class Stream extends Furo {
  ...
  public constructor({
    ...
    this.initialSharesExtended = Amount.fromRawAmount(
      this.token,
      BigInt(furo.initialSharesExtended),
    )
    this.extendedShares = Amount.fromRawAmount(
      this.token,
      BigInt(furo.extendedShares),
    )
    this.extendedAtTimestamp = new Date(
      parseInt(furo.extendedAtTimestamp) * 1000,
    )
    this.withdrawnShares = Amount.fromRawAmount(
      this.token,
      this.initialShares.quotient +
        this.extendedShares.quotient -
        this.remainingShares.quotient,
    )
    this.withdrawnAmountAfterExtension = Amount.fromRawAmount(
      this.token,
      furo.withdrawnAmountAfterExtension,
    )
  }
  ...
  public override get balance(): Amount<Token> {
    ...
    if (this.extendedShares.equalTo(0)) {
      const duration =
        BigInt(this.endTime.getTime()) - BigInt(this.startTime.getTime())
      ...
      const pendingAmount = Amount.fromShare(
        this.token,
        streamedShares - this.withdrawnShares.quotient,
        this.rebase,
      )
      const balance = Amount.fromRawAmount(
        this.token,
        this.withdrawnAmount.quotient + pendingAmount.quotient,
      )

      return balance.lessThan(this.totalAmount) ? balance : this.totalAmount
    } else {
      const duration =
        BigInt(this.endTime.getTime()) -
        BigInt(this.extendedAtTimestamp.getTime())
      const passed =
        BigInt(Date.now()) - BigInt(this.extendedAtTimestamp.getTime())
      const streamedSharesAfterExtension =
        (this.initialSharesExtended.quotient * passed) / duration
      const withdrawnSharesAfterExtension =
        this.initialSharesExtended.quotient - this.remainingShares.quotient
      const pendingAmount = Amount.fromShare(
        this.token,
  ...
  public get streamedPercentage(): Percent | undefined {
    ...
    if (this.extendedShares.equalTo(0)) {
      ...
      const totalShares =
        this.initialShares.quotient + this.extendedShares.quotient
      const streamedBeforeExtension =
        totalShares - this.initialSharesExtended.quotient
      ...
      const totalStreamed =
        streamedSharesAfterExtension + streamedBeforeExtension
