...
export class Vesting extends Furo {
  ...
  public constructor({
    ...
    this.cliffShares = Amount.fromRawAmount(
      this.token,
      JSBI.BigInt(vesting.cliffShares)
    );
    ...
    this.stepShares = Amount.fromRawAmount(
      this.token,
      JSBI.BigInt(vesting.stepShares)
    );
    ...
    this.totalAmount = Amount.fromRawAmount(
      this.token,
      JSBI.add(
        JSBI.BigInt(this._remainingAmount.quotient),
        JSBI.BigInt(vesting.withdrawnAmount)
      )
    );
  ...
  public get balance(): Amount<Token> {
    const timeAfterCliff = JSBI.add(
      JSBI.BigInt(this.startTime.getTime()),
      JSBI.BigInt(this.cliffDuration)
    );
    ...
    const stepPassed = minimum(
      JSBI.BigInt(this.steps),
      JSBI.divide(passedSinceCliff, JSBI.BigInt(this.stepDuration))
    );
    ...
    return Amount.fromRawAmount(
      ...
      JSBI.subtract(
        JSBI.add(
          ...
          JSBI.multiply(this.stepAmount.quotient, stepPassed)
        ),
  ...
  public get nextPaymentTimeRemaining():
    ...
    if (
      ...
      if (this.vestingType === VestingType.GRADED) {
        ...
        if (this.status === FuroStatus.UPCOMING) {
          ...
          const cliffTime =
            this.startTime.getTime() + this.cliffDuration * 1000;
          if (now <= cliffTime) {
            interval =
              this.startTime.getTime() + this.cliffDuration * 1000 - now;
          } else {
            const totalDuration =
              this.endTime.getTime() - this.cliffDuration * 1000 - now;
            interval = totalDuration % (this.stepDuration * 1000);
          }
        }
      } else {
