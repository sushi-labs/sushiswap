        ...
        <div className="max-w-[530px] gap-4 flex flex-col rounded-lg overflow-hidden">
          <div className="flex justify-between gap-4 p-4 bg-slate-700">
            <div className="flex gap-6">
              <div className="flex items-center justify-end gap-2">
                ...
                <Typography
                  ...
                  variant="xs"
                  onClick={() =>
                    stream && navigator.clipboard.writeText(stream.createdBy.id)
                  }
              ...
              <div className="flex items-center justify-end gap-2">
                ...
                <Typography
                  ...
                  variant="xs"
                  onClick={() =>
                    stream && navigator.clipboard.writeText(stream.recipient.id)
                  }
                  className="hover:text-blue"
                >
            ...
            <a
              target="_blank"
              href={Chain.from(
                stream.remainingAmount.currency.chainId
              )?.getTxUrl(stream.txHash)}
              rel="noreferrer"
            ...
            {stream?.status !== FuroStatus.CANCELLED ? (
              <>
                <Typography
                  variant="xxs"
                  weight={400}
                  className="tracking-wider uppercase text-slate-500"
                >
                  Time Remaining
                </Typography>
                ...
                >
                  ...
                  <span className="text-slate-200">
                    {format(new Date(stream.startTime), "dd MMM yyyy hh:mmaaa")}
                  </span>{" "}
                  If the sender does not cancel the stream, the full amount will
                  be disbursed to you on{" "}
                  <span className="text-slate-200">
                    {format(new Date(stream.endTime), "dd MMM yyyy hh:mmaaa")}
                  </span>
                </Typography>
              </>
            ) : (
          ...
          ${stream?.activeTime?.days} days ${stream?.activeTime?.hours} hours ${
                    stream?.activeTime?.minutes
                  } minutes. `}
                </Typography>
              </>
            )}
          ...
          <div className="grid grid-cols-4 gap-4 px-4 pb-4 sm:grid-cols-3">
            <div className="flex flex-col col-span-4 gap-4 p-4 sm:col-span-1 bg-slate-700 rounded-xl">
              <div className="flex flex-col">
                ...
                <Typography
                  variant="h3"
                  className="flex items-center mt-3 truncate text-slate-200"
                  weight={500}
                >
            ...
            <div className="flex flex-col col-span-2 gap-4 p-4 sm:col-span-1 bg-blue/30 rounded-xl">
              <div className="flex flex-col">
                ...
                <Typography
                  variant="h3"
                  className="flex items-center mt-3 text-blue-200 truncate"
                  weight={500}
                >
            ...
            <div className="flex flex-col col-span-2 gap-4 p-4 shadow-md sm:col-span-1 bg-pink/30 rounded-xl">
              <div className="flex flex-col">
                ...
                <Typography
                  variant="h3"
                  className="flex items-center mt-3 text-pink-200 truncate"
                  weight={500}
                >
                ...
                <Typography
                  ...
                  weight={500}
                  as="span"
                  className="mt-1 text-pink-300/80"
                >
                  {stream?.withdrawnAmount.currency.symbol}
                </Typography>
              </div>
