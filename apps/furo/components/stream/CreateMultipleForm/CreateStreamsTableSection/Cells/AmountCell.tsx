...
export const AmountCell: FC<CellProps> = ({
  row,
  index,
  ...
  const _fundSource =
    ZFundSourceToFundSource.parse(row.fundSource) || FundSource.WALLET;
  const _amount = useMemo(
    () => tryParseAmount(row.amount, _currency),
  ...
  useEffect(() => {
    if (
      ...
      balance &&
      balance[_fundSource] &&
      balance[_fundSource].lessThan(_amount)
    ) {
          ...
          <div className="py-2">
            <Input.Numeric
              ...
              className={classNames(
                error?.message ||
                  errors?.["FORM_ERRORS"]?.[index]?.["amount"]?.message
                  ? "!border-red"
