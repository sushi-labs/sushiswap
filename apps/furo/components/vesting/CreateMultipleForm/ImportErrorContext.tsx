...
export const ImportErrorProvider = <T extends FieldValues>({
  children,
}: ImportErrorContextProps) => {
  const [errors, setErrors] = useState<FieldErrors<T>>({});

  return (
    <ImportErrorContext.Provider
      value={useMemo(() => ({ errors, setErrors }), [errors])}
    >
      {children}
    </ImportErrorContext.Provider>
  );
...
export const useImportErrorContext = <T extends FieldValues>() => {
  const context = useContext<State<T>>(
    ImportErrorContext as unknown as React.Context<State<T>>
  );
  if (!context) {
