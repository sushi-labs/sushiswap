        ...
        <div className="overflow-hidden rounded-lg flex flex-col max-w-[530px] bg-slate-800">
          <div className="p-4 bg-slate-700">
              ...
              <Typography variant="xs" className="!leading-5 text-slate-300">
                ...
                <span className="font-medium text-slate-200 hover:text-blue">
                  <a
                    target="_blank"
                    href={Chain.from(
                      vesting.remainingAmount.currency.chainId
                    )?.getAccountUrl(vesting.recipient.id)}
                    rel="noreferrer"
                  >
                ...
                <span className="font-medium text-slate-200">
                  ...
                  {vesting.remainingAmount?.currency.symbol}
                </span>{" "}
                <br /> runs from{" "}
                <span className="font-medium text-slate-200">
            ...
            <div className="mt-6 grid grid-cols-[60px_80px_80px_auto] gap-2 items-center">
              <Typography
                weight={500}
                className="tracking-wider capitalize text-slate-400"
                variant="xxs"
              >
                Schedule
              </Typography>
              ...
              <Typography
                weight={500}
                className="tracking-wider text-right capitalize text-slate-400"
                variant="xxs"
              >
                Amount
              </Typography>
          ...
          <div className="px-4 overflow-auto max-h-[240px] hide-scrollbar divide-y divide-slate-700/50">
            {schedule?.length ? (
              ...
              <Typography
                variant="xs"
                className="flex items-center justify-center h-full py-4 italic text-slate-500"
              >
                No schedule found
              </Typography>
            )}
...
const SchedulePopoverItem: FC<{ vesting?: Vesting; period: Period }> = memo(
  ({ vesting, period }) => {
    return (
      <div
        key={period.id}
        className="py-2 grid grid-cols-[60px_80px_80px_auto] gap-2 items-center"
      >
        <Typography
          className="tracking-wider capitalize text-slate-200"
          weight={500}
          variant="xxs"
        >
        ...
        <Typography
          variant="xs"
          className="flex flex-col text-left text-slate-200"
          weight={500}
        >
        ...
        <Typography
          variant="xs"
          className="flex flex-col text-right truncate text-slate-200"
          weight={500}
        >
          {period.type === PeriodType.START
            ...
            : period.type === PeriodType.END &&
              (vesting?.vestingType === VestingType.GRADED ||
                vesting?.vestingType === VestingType.HYBRID)
            ? vesting?.stepAmount.toSignificant(6)
            : vesting?.remainingAmount.toSignificant(6)}
          <Typography as="span" variant="xxs" className="text-slate-500">
