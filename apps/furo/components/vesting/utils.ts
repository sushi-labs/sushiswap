...
export const calculateEndDate = ({
  ...
  "cliff" | "startDate" | "stepPayouts" | "stepConfig"
>) => {
  if (!stepPayouts || !stepConfig) return undefined;
  if (startDate && !cliff.cliffEnabled)
    return new Date(startDate.getTime() + stepConfig.time * stepPayouts * 1000);
  if (cliff.cliffEnabled && cliff.cliffEndDate)
    return new Date(
      cliff.cliffEndDate.getTime() + stepConfig.time * stepPayouts * 1000
    );
  return undefined;
};
...
export const calculateCliffDuration = ({
  ...
  if (
    ...
    cliff.cliffEndDate &&
    startDate &&
    cliff.cliffEndDate > startDate
  ) {
    cliffDuration = JSBI.BigInt(
      Math.floor((cliff.cliffEndDate.getTime() - startDate.getTime()) / 1000)
    );
  }
...
>) => {
  ...
  return totalAmount?.greaterThan(ZERO) && _stepAmount
    ? new Fraction(
        _stepAmount.multiply(JSBI.BigInt(1e18)).quotient,
