...
export const GradedVestingDetailsSection = () => {
  ...
  const { currency, stepConfig, fundSource, cliff, stepAmount, stepPayouts } =
    _formData;
  ...
  useEffect(() => {
    if (!_fundSource || !totalAmount || !balance || !balance[_fundSource])
      return;
  ...
  return (
    <Form.Section
      title="Graded Vesting Details"
      description="Optionally provide graded vesting details"
    >
                  ...
                  text={
                    error?.message ? (
                      ...
                      <>
                        ...
                        For a value of {value} and a{" "}
                        {stepConfig?.label.toLowerCase()} period length, the
                        user will receive {value} {currency?.symbol}{" "}
        ...
        <Form.Control label="Total Amount">
          <Typography
            ...
            className={
              _fundSource &&
              balance?.[_fundSource] &&
              totalAmount?.greaterThan(balance[_fundSource])
                ? "text-red"
                : totalAmount
                ? "text-slate-50"
                : "text-slate-500"
            ...
            weight={600}
          >
            {totalAmount ? totalAmount?.toSignificant(6) : "0.000000"}{" "}
