...
const Table: FC<{
  ...
  return (
    <div className={classNames(className, "flex flex-col pb-3 gap-2")}>
      <Typography
        variant="xxs"
        className="!leading-5 tracking-widest text-slate-400 font-semibold uppercase"
      >
        {title}
      </Typography>
...
const CreateFormReviewModalBase: FC<CreateFormReviewModalBase> = ({
  chainId,
  children,
  open,
  setOpen,
      ...
      <Dialog.Content className="!space-y- min-h-[300px] !max-w-md relative overflow-hidden bg-slate-900 !pb-3 !px-4">
        ...
        {_currency && recipient && isAddress(recipient) && endDate && (
          <>
            <Typography
              variant="xs"
              className="!leading-5 text-slate-400 mt-6 text-center"
            >
              ...
              from{" "}
              <span className="font-medium text-slate-50">
                {!!startDate && format(startDate, "dd MMM yyyy hh:mm")}
              </span>{" "}
              until{" "}
              <span className="font-medium text-slate-50">
            ...
            <div className="flex flex-col gap-2 mt-6">
              ...
              <Table
                title="Cliff Details"
                className={cliff.cliffEnabled ? "" : "opacity-40"}
              >
                {cliff.cliffEnabled && cliff.cliffEndDate ? (
                  <>
                    <Item
                      title="Cliff End Date"
                      value={format(cliff.cliffEndDate, "dd MMM yyyy")}
                    />
                    <Item
                      title="Cliff Amount"
                      value={
                  ...
                  <Item
                    title="Cliff Amount"
                    value={<span className="italic">Not enabled</span>}
                  />
                )}
              </Table>
