...
export const Dashboard: FC<{
  ...
  const { data: vestings, isValidating: isValidatingVestings } =
    useSWR<Vestings>(`/furo/api/user/${chainId}/${address}/vestings`, fetcher);
  ...
  const { data: rebases, isValidating: isValidatingRebases } = useSWR<Rebase[]>(
    () =>
      streams
        ? [
            `/furo/api/rebases/${chainId}/${tokens
  ...
  const { isLoading: balancesLoading, data: balancesData } = useStreamBalances(
    chainId,
    ids,
    tokens,
    {
      ...
      <div className="flex justify-between">
        <div className="flex flex-col gap-3">
          ...
          <Typography className="text-slate-400">
            Earn, stream and automate your DAO salaries <br /> and token vesting
            with Furo.
          </Typography>
          ...
          <Tab.List className="flex gap-10">
            ...
            >
              ...
              <Chip
                color="blue"
                label={(
                  (streams?.incomingStreams?.length || 0) +
                  (vestings?.incomingVestings?.length || 0)
                ).toString()}
              />
            </Tab>
            <Tab
            ...
            >
              ...
              <Chip
                color="pink"
                label={(
                  (streams?.outgoingStreams?.length || 0) +
                  (vestings?.outgoingVestings?.length || 0)
                ).toString()}
              />
            </Tab>
          </Tab.List>
