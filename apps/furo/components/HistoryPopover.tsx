...
export const HistoryPopover: FC<Props> = ({
  stream,
  ...
  const transactions = useMemo(
    () =>
      stream
        ? transactionRepresentations?.map(
            (transaction) => new Transaction(transaction, stream.chainId)
          )
        : [],
        ...
        <div className="flex flex-col overflow-hidden rounded-lg bg-slate-800">
          <div className="p-4 bg-slate-700">
            <div className="grid grid-cols-[80px_80px_100px] gap-2 items-center">
              ...
              <Typography
                weight={500}
                className="tracking-wider text-left capitalize text-slate-400"
                variant="xxs"
              >
                Date
              </Typography>
...
const HistoryPopoverTransaction: FC<{
  ...
  return (
    <div
      key={transaction.id}
      className="py-2 grid grid-cols-[80px_80px_100px] gap-2 items-center"
    >
      <Typography
      ...
      <Typography
        variant="xs"
        weight={500}
        className="flex flex-col text-right text-slate-200"
      >
