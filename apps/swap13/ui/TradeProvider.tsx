  ...
  switch (action.type) {
    ...
    case "setToken0":
      return {
        ...
        token1:
          state.token1.wrapped.address === action.currency.wrapped.address
            ? state.token0
            : state.token1,
      };
    case "setToken1":
      return {
        ...
        token0:
          state.token0.wrapped.address === action.currency.wrapped.address
            ? state.token1
            : state.token0,
...
export const SwapProvider: FC<SwapProviderProps> = ({ children }) => {
  ...
  const api = useMemo(() => {
    const setNetwork0 = (chainId: ChainId) =>
      dispatch({ type: "setNetwork0", chainId });
    const setNetwork1 = (chainId: ChainId) =>
      dispatch({ type: "setNetwork1", chainId });
    const setToken0 = (currency: Type) =>
      dispatch({ type: "setToken0", currency });
    const setToken1 = (currency: Type) =>
      dispatch({ type: "setToken1", currency });
    const setAppType = (appType: AppType) =>
      dispatch({ type: "setAppType", appType });
  ...
  return (
    <APIContext.Provider value={api}>
      <DataContext.Provider
        value={useMemo(
          () => ({ ...state, recipient: state.recipient ?? address }),
          [address, state]
        )}
      >
        {children}
