...
import { useSupplyAPR, useTokensFromKashiPair } from '../../../lib/hooks'

interface LendTableHoverElementProps {
  row: KashiPair
}

const Item = ({ pair }: { pair: KashiPair }) => {
  const apr = useSupplyAPR(pair)
  return (
  ...
  return (
    <div className="p-3 overflow-hidden rounded-md">
      ...
      <div className="flex flex-col gap-2">
        {pairs
          ? pairs.map((pair, i) => <Item key={i} pair={pair} />)
