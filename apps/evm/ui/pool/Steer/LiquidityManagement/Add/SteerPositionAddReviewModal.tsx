...
export const SteerPositionAddReviewModal: FC<SteerPositionAddReviewModalProps> = ({
  ...
  const { data: accountPosition } = useSteerAccountPosition({ account: address, vaultId: vault.id })
  const accountPositionAmountsArray = useMemo(() => {
    if (!currencies || !accountPosition) return null

    const token0Amount = Amount.fromRawAmount(currencies.CURRENCY_A, accountPosition.token0Balance)
    const token1Amount = Amount.fromRawAmount(currencies.CURRENCY_B, accountPosition.token1Balance)

    return [token0Amount, token1Amount]
  }, [accountPosition, currencies])
  const accountPositionValues = useTokenAmountDollarValues({ chainId: chainId, amounts: accountPositionAmountsArray })
  ...
  const parsedAmountsArray = useMemo(
    () => (parsedAmounts ? [parsedAmounts?.CURRENCY_A, parsedAmounts?.CURRENCY_B] : null),
    [parsedAmounts]
  )
  const newPositionValues = useTokenAmountDollarValues({ chainId: chainId, amounts: parsedAmountsArray })
  const newPositionValue = useMemo(() => newPositionValues.reduce((acc, cur) => acc + cur, 0), [newPositionValues])
      ...
      <DialogReview>
        {({ confirm }) => (
          <>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>{SteerStrategyConfig[vault.strategy].name}</DialogTitle>
                ...
                <List className="!pt-0">
                  <List.Control>
                    {currencies && parsedAmounts && (
                      <>
                        <List.KeyValue flex title={`${currencies.CURRENCY_A.symbol}`}>
                          <div className="flex items-center gap-2">
                            <Currency.Icon currency={currencies.CURRENCY_A} width={18} height={18} />
                            {parsedAmounts.CURRENCY_A?.toSignificant(6)} {currencies.CURRENCY_A.symbol}
                          </div>
                        </List.KeyValue>

                        <List.KeyValue flex title={`${currencies.CURRENCY_B.symbol}`}>
                          <div className="flex items-center gap-2">
                            <Currency.Icon currency={currencies.CURRENCY_B} width={18} height={18} />
                            {parsedAmounts.CURRENCY_B.toSignificant(6)} {currencies.CURRENCY_B.symbol}
                          </div>
                        </List.KeyValue>
                      </>
                    )}
