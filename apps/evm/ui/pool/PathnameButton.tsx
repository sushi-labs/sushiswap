...
const PathnameButton = forwardRef<HTMLButtonElement, PathnameButton>(({ pathname, ...props }, ref) => {
  const _pathname = usePathname()
  return (
    <Button
      {...props}
      ref={ref}
      variant={_pathname === pathname || _pathname === pathname.replace('%3A', ':') ? 'secondary' : 'ghost'}
    />
  )
})
