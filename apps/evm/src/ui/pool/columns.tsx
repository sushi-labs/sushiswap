  ...
  cell: ({ row }) => {
    switch (row.original.type) {
      case TransactionType.Swap:
        return (
          <span>
            <FormattedNumber number={row.original.amount0In !== "0" ? row.original.amount0In : row.original.amount1In} />{' '}
            { row.original.amount0In !== "0" ? row.original.pool.token0.symbol : row.original.pool.token1.symbol }
          </span>
        )
    ...
    switch (row.original.type) {
      case TransactionType.Swap:
        return (
          <span>
            <FormattedNumber
              number={Math.abs(row.original.amount0Out !== "0" ? row.original.amount0Out : row.original.amount1Out).toPrecision(2)}
            />{' '}
            { row.original.amount0Out  !== "0" ? row.original.pool.token0.symbol : row.original.pool.token1.symbol }
          </span>
        )
      case TransactionType.Mint:
      case TransactionType.Burn:
