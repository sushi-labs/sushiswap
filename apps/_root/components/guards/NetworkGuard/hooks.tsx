...
import NETWORKS, { ChainsWithData } from 'chain/networkConfig'
...
export const useNetworkHandlers = () => {
  ...
  const addNetwork = useCallback(
    async (chainId: ChainsWithData, account?: string) => {
      if (!provider || !account) return console.error('Dependencies unavailable')
  ...
  const switchNetwork = useCallback(
    (chainId: number, account?: string) => {
      ...
      provider.send('wallet_switchEthereumChain', [{ chainId: `0x${chainId.toString(16)}` }, account]).catch((e) => {
