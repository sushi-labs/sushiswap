...
export default async function getBentoTVL() {
  ...
  const chainIds = Object.keys(BENTOBOX_SUBGRAPH_NAME).map(
    (chainId) => Number(chainId) as BentoBoxChainId
  );
          ...
          return bentoTokens
            .map((bentoToken: any) => {
              const tokenPriceUSD =
                Number(
                  tokenPrices.find(
                    (tokenPrice: any) => tokenPrice.id === bentoToken.id
                  )?.derivedETH ?? 0
                ) * nativePrice;

              return Number(
                (bentoToken.rebase.elastic / 10 ** bentoToken.decimals) *
                  tokenPriceUSD
              );
            })
  ...
  return (await Promise.all(bentoTVLQueries)).reduce(
    (acc, cur) => acc + cur,
    0
  );
}
