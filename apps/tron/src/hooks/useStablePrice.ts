import { STABLE_TOKENS, WTRX } from "src/constants/token-list";
import { IToken } from "../types/token-type";
import { getToken0Price, getToken1Price, getValidTokenAddress } from "src/utils/helpers";
import { getAllPairAddresses } from "src/lib/getAllPairAddresses";
import { IReserveDataResponse } from "src/types/get-pools-type";
import { getTronPrice } from "src/lib/getTronPrice";
...
const getBestStable = (stablePairs: Pair) => {
	const bestStable = stablePairs.find(
		(pair) => pair.token0Address === BEST_STABLE?.address || pair.token1Address === BEST_STABLE?.address
	);
	if (bestStable) {
		return bestStable;
	} else {
...
const getTokenPrice = async (token: IToken | undefined) => {
	...
	const isNativeOrWrappedTrx =
		getValidTokenAddress(token.address).toLowerCase() === WTRX.address.toLowerCase();
	...
	if (stablePairs?.length > 0) {
		...
		if (token.address === pair.token0Address) {
			...
			return getToken0Price(reserve0, reserve1, token.decimals, token1Data?.decimals);
			...
			return getToken1Price(reserve0, reserve1, token0Data?.decimals, token.decimals);
		}
	}
	...
	if (wtrxPairs?.length > 0) {
		...
		const { reserve0, reserve1 } = await getReserves(pair.pairAddress);
		const tronInUSDT = await getTronPrice();
		if (getValidTokenAddress(pair.token0Address).toLowerCase() === WTRX.address.toLowerCase()) {
	...
	try {
		...
		const stablePairsThatIncludeToken = allPairs.filter(
			(pair) =>
				...
				(getValidTokenAddress(pair.token0Address).toLowerCase() === token.address.toLowerCase() ||
					getValidTokenAddress(pair.token1Address).toLowerCase() === token.address.toLowerCase())
		);
		const wtrxPairsThatIncludeToken = allPairs.filter(
			(pair) =>
				(getValidTokenAddress(pair.token0Address).toLowerCase() === WTRX.address.toLowerCase() ||
					getValidTokenAddress(pair.token1Address).toLowerCase() === WTRX.address.toLowerCase()) &&
				(getValidTokenAddress(pair.token0Address).toLowerCase() === token.address.toLowerCase() ||
					getValidTokenAddress(pair.token1Address).toLowerCase() === token.address.toLowerCase())
		);
		return {
...
const getReserves = async (pairAddress: string) => {
	try {
		const res = await fetch(`/api/pools/get-reserves?pairAddresses=${[pairAddress]}`, { method: "GET" });
		if (!res.ok) {
		...
		return {
			reserve0:
				data?.data?.tron.smartContractEvents[0].arguments.find((arg) => arg.argument === "reserve0")?.value ??
				"",
			reserve1:
				data?.data?.tron.smartContractEvents[0].arguments.find((arg) => arg.argument === "reserve1")?.value ??
				"",
		};
	} catch (error) {
