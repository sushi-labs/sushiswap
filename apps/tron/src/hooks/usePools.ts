...
import { IPoolDataResponse, IReserveDataResponse } from "src/types/get-pools-type";
...
const getPoolsByEvent = async ({ factoryAddress }: { factoryAddress: string }): Promise<_IPools> => {
	try {
		const res = await fetch(`/api/pools?factoryAddress=${factoryAddress}`, { method: "GET" });
		if (!res.ok) {
		...
		for (const chunk of chunkedPools) {
			...
			for (const pool of chunk) {
				...
				if (reserveData) {
					...
					const originalPool = poolsCopy.find((p) => p.pairAddress === pool.pairAddress);
					if (originalPool) {
