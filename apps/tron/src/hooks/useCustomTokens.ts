...
const localStorageKey = IS_TESTNET ? "sushi.customTokens.shasta" : "sushi.customTokens.tron";

export const useCustomTokens = () => {
	const [value, setValue] = useLocalStorage<Record<string, IToken>>(localStorageKey, {});
	...
	const removeCustomToken = useCallback(
		(currency: IToken) => {
			setValue((prev) => {
				return Object.entries(prev).reduce<Record<string, IToken>>((acc, cur) => {
