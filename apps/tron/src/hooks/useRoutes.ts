...
import { INTERMEDIATE_TOKEN } from "src/constants/token-list";
import { FACTORY_CONTRACT, MULTICALL_CONTRACT } from "src/constants/contracts";
import { TRON_MULTICALL_ABI } from "src/constants/abis/tron-multicall";
import { ethers } from "ethers";
import { getBase58Address, getIfWrapOrUnwrap, getValidTokenAddress, isContract } from "src/utils/helpers";
...
const getPairsToCall = (token0Address: string, token1Address: string, intermediateTokenAddress: string) => {
	return [
...
export const useRoutes = ({ token0, token1 }: { token0: IToken; token1: IToken }) => {
	...
	return useQuery({
		...
		queryFn: async () => {
			...
			const pairsToCall = getPairsToCall(token0Address, token1Address, intermediateTokenAddress);

			const calls = pairsToCall.map(({ token0Address, token1Address }) => {
				const parameters = tronWeb.utils.abi.encodeParamsV2ByABI(abi, [token0Address, token1Address]);
			...
			for (let i = 0; i < returnData.length; i++) {
				const pairAddress = ethers.utils.defaultAbiCoder.decode(["address"], returnData[i]);
			...
			if ((await isContract(tronWeb, _pairs?.[1])) && (await isContract(tronWeb, _pairs?.[2]))) {
				return {
