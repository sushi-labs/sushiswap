...
export function createTokenFilterFunction<T extends IToken>(search: string): (tokens: T) => boolean {
...
export function filterTokens<T extends IToken>(tokens: T[], search: string): T[] {
...
export function getSortedTokensByQuery(tokens: IToken[] | undefined, searchQuery: string): IToken[] {
	if (!tokens) {
		return [];
	}
