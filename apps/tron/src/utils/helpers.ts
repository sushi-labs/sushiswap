...
} from "src/types/router-selector-type";
...
export const getIfWrapOrUnwrap = (token0: IToken, token1: IToken): "wrap" | "unwrap" | "swap" => {
...
export const getTransactionInfo = async (
	tronWebInstance: any,
	txId: string,
	waitIntervalMs: number = 6000,
	maxTries: number = 15
) => {
	//avg txn is 1min till confirmed
	let transactionInfo;
	for (let i = 0; i < maxTries; i++) {
		transactionInfo = await tronWebInstance.trx.getUnconfirmedTransactionInfo(txId);
...
//TODO: Add type for result
export const safeGasEstimates = async (tronWebInstance: any, args: any[][]): Promise<any[]> => {
...
export const getDeadline = (deadlineInMinutes: number = 20): number => {
	const currentTimestamp = Math.floor(Date.now() / 1000);
	const bufferTime = deadlineInMinutes * 60;
	return Math.floor(currentTimestamp + bufferTime);
};

export const getRouterFunctionSelectors = (route: string[]): IRouterFunctionSelector[] => {
...
export const getArgsForAddLiquidity = (
	...
	switch (functionSelector) {
		...
		case "addLiquidityETH(address,uint256,uint256,uint256,address,uint256)":
			const tokenAddress = token0Address === "TRON" ? token1Address : token0Address;
			const tokenAmount = token0Address === "TRON" ? amount1Desired : amount0Desired;
			const tokenMinAmount = token0Address === "TRON" ? amount1Min : amount0Min;
			const ethAmount = token0Address === "TRON" ? amount0Desired : amount1Desired;
...
export const getRemoveLiquidityFunctionSelector = (
	...
	return ["removeLiquidity(address,address,uint256,uint256,uint256,address,uint256)"];
};
...
export const getArgsForRemoveLiquidity = (
	...
	switch (functionSelector) {
		...
		case "removeLiquidityETH(address,uint256,uint256,uint256,address,uint256)":
			const tokenAddress = token0Address === "TRON" ? token1Address : token0Address;
			const tokenMinAmount = token0Address === "TRON" ? amountToken1Min : amountToken0Min;
			const ethMinAmount = token0Address === "TRON" ? amountToken0Min : amountToken1Min;
			return [
				ROUTER_CONTRACT,
				functionSelector,
				{},
		...
		case "removeLiquidityETHSupportingFeeOnTransferTokens(address,uint256,uint256,uint256,address,uint256)":
			const _tokenAddress = token0Address === "TRON" ? token1Address : token0Address;
			const _tokenMinAmount = token0Address === "TRON" ? amountToken1Min : amountToken0Min;
			const _ethMinAmount = token0Address === "TRON" ? amountToken0Min : amountToken1Min;
			return [
				ROUTER_CONTRACT,
				functionSelector,
				{},
