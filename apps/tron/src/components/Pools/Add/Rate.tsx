...
import { formatUnits, parseUnits } from "src/utils/formatters";
import { getToken0AmountForLiquidity, getToken1AmountForLiquidity } from "src/utils/helpers";
...
export const Rate = ({
	...
	const rateOfToken0 = useMemo(() => {
		...
		return getToken0AmountForLiquidity(parseUnits("1", token1?.decimals), reserve0, reserve1);
	}, [token0, token1, reserve0, reserve1]);

	const rateOfToken1 = useMemo(() => {
		if (!reserve0 || !reserve1) return;
		if (!token0 || !token1) return;
		return getToken1AmountForLiquidity(parseUnits("1", token0?.decimals), reserve0, reserve1);
	}, [token0, token1, reserve0, reserve1]);
