...
export const ReviewAddDialogTrigger = () => {
	const { token0, token1, isTxnPending, amountInToken0, amountInToken1 } = usePoolState();
	const { address } = useWallet();

	const { data: token0Balance, isLoading: isLoadingToken0Balance } = useTokenBalance({
		accountAddress: address,
		tokenAddress: token0?.address,
	});

	const { data: token1Balance, isLoading: isLoadingToken1Balance } = useTokenBalance({
		accountAddress: address,
		tokenAddress: token1?.address,
	});

	const { data: token0AllowanceAmount, refetch: refetchToken0Allowance } = useAllowance({
		tokenAddress: token0?.address as string,
		ownerAddress: address as string,
		spenderAddress: ROUTER_CONTRACT,
	});

	const { data: token1AllowanceAmount, refetch: refetchToken1Allowance } = useAllowance({
	...
	const hasInsufficientToken0Balance = useMemo(() => {
		...
		return Number(formatUnitsForInput(token0Balance ?? "0", token0?.decimals ?? 18)) < Number(amountInToken0);
	...
	const hasInsufficientToken1Balance = useMemo(() => {
		...
		return Number(formatUnitsForInput(token1Balance ?? "0", token1?.decimals ?? 18)) < Number(amountInToken1);
	}, [token1Balance, token1, amountInToken1, isLoadingToken1Balance]);

	const token0AllowanceFormatted = formatUnitsForInput(token0AllowanceAmount ?? "0", token0?.decimals ?? 18);

	const token1AllowanceFormatted = formatUnitsForInput(token1AllowanceAmount ?? "0", token1?.decimals ?? 18);

	const hasInsufficientToken0Allowance = useMemo(() => {
		return !!token1AllowanceAmount && Number(token0AllowanceFormatted) < Number(amountInToken0);
	}, [token0AllowanceFormatted, amountInToken0, token1AllowanceAmount]);

	const hasInsufficientToken1Allowance = useMemo(() => {
		return !!token1AllowanceAmount && Number(token1AllowanceFormatted) < Number(amountInToken1);
	...
	return (
		<DialogTrigger
			disabled={invalidAmount || hasInsufficientToken0Balance || hasInsufficientToken1Balance || isTxnPending}
			asChild>
