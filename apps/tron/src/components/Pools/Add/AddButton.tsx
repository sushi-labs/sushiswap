...
export const AddButton = ({ closeModal }: { closeModal: () => void }) => {
	...
	const { token0, token1, isTxnPending, amountInToken0, amountInToken1, pairAddress } = usePoolState();
	const { setIsTxnPending, setAmountInToken0, setAmountInToken1 } = usePoolDispatch();
	const { address, signTransaction } = useWallet();
	const { tronWeb } = useTronWeb();
	const [slippageTolerance] = useSlippageTolerance("sushi-tron-slippage-add-liq");
	...
	const addLiquidity = async () => {
		if (!token0 || !token1 || !amountInToken0 || !amountInToken1 || !address) return;
		try {
			...
			const { transaction } = await tronWeb.transactionBuilder.triggerSmartContract(...cleanedArgs);
			...
			const errorMessage =
				typeof error === "string"
					? error
					: (error as Error)?.message ?? "An error occurred while trying to add liquidity";
			//create error toast
			createFailedToast({
