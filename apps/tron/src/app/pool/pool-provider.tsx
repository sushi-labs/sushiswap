...
import { ReserveHelper } from "src/components/Pools/ReserveHelper";
	...
	switch (action.type) {
		case "setToken0": {
			...
			if (
				...
				return { ..._state, token0: DEFAULT_TOKEN_LIST[0], token1: DEFAULT_TOKEN_LIST[2] };
			}
...
const PoolProvider: FC<PoolProviderProps> = ({ children }) => {
	...
	const dispatchWithAction = useMemo(
		() => ({
			...
			setPairAddress: (value: string | undefined | null) => dispatch({ type: "setPairAddress", value }),
