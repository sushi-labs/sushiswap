...
export const Header: FC = () => {
  const { data: productsData } = useSWR(
    "/products",
    async () => (await getProducts())?.products
  );
          ...
          return (
            <Select
              ...
              button={
                <Listbox.Button
                  type="button"
                  className="flex items-center gap-1 font-medium text-slate-50"
                >
                ...
                {links?.map(({ name, href, isExternal }) =>
                  isExternal ? (
                    <Link.External key={href} href={href}>
                      <Select.Option
                        value={name}
                        className="pr-10 border-0 !cursor-pointer"
                      >
                        {name}
                      </Select.Option>
                    </Link.External>
