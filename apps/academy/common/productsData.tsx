    ...
    cards: [
      {
        Icon: () => (
          <AcademyIcon
            color={PRODUCTS_DATA.miso.accentColor}
            Icon={ThreeLinkIcon}
          />
        ),
        title: "Wide Exposure",
        subtitle:
          "Introduce your tokens to the largest possible crypto native audiences.",
      },
      {
        Icon: () => (
          <AcademyIcon
        ...
        subtitle:
          "Anyone can launch a token on MISO, no technical knowledge required.",
      },
      {
        Icon: () => (
          <AcademyIcon color={PRODUCTS_DATA.miso.accentColor} Icon={BarsIcon} />
        ),
        title: "Rich in Data",
        subtitle:
          "The highest level of transparency and data availability to help you make informed decisions.",
      },
      {
        Icon: () => (
          <AcademyIcon
    ...
    faq: [
      {
        ...
        answer: (
          <div className="space-y-4">
            <p>
              Dutch auctions are “inverted” auctions. The price point per token
              is set high and descends as the auction goes on in the event that
              no one is buying in. This rate of price adjustment is set by the
              auctioneer. The final price per token is set when all tokens have
              been allocated. Participants are committing to receive a minimum
              total value rather than bidding for the tokens themselves -
              because of this it is common to end up with more tokens than what
              was initially bid for.
            </p>
            <p>
              <i>Example:</i> An auction starts at $100 per token. It is set to
              depreciate in value every hour if a token is not bought. If no one
              buys the token after the first hour, it automatically lowers
              itself to the price point of $90 per token. Every purchase resets
              the hour long timer until the token price starts to depreciates
              again. If no tokens are bought in the second hour - again the
              price of the token drops, this time to $80. Another hour passes,
              now it is valued at $70. This cycle would continue until all the
              tokens are purchased or they become valued at $0.
            </p>
          </div>
        ),
      },
      {
        ...
        answer: (
          <p>
            Batch auctions are based around a single pool of tokens that is
            split amongst all auction participants. At the end of the auction
            the token price is determined by the total amount raised from all
            participants. Everybody participating gets a share of the tokens
            equal to their share of the contribution pool.
          </p>
        ),
      },
    ],
