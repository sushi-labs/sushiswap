...
if (!process.env.PLAYWRIGHT_URL)
  throw new Error("PLAYWRIGHT_URL env var not set");
...
async function wrap(trade: Trade, page: Page, useBalance?: boolean) {
  ...
  const unwrapButton = page.locator(
    "[testdata-id=open-wrap-review-modal-button]"
  );
  await expect(unwrapButton).toBeEnabled();
  await unwrapButton.click();

  const confirmUnwrap = page.locator(
    "[testdata-id=swap-wrap-review-modal-confirm-button]"
  );
  ...
  await expect(
    page.locator("div", { hasText: expectedRegex }).last()
  ).toContainText(expectedRegex);
}
...
async function swap(trade: Trade, page: Page, useMaxBalances?: boolean) {
  ...
  await page
    ...
    .catch(() =>
      console.log(`${trade.input.symbol} already approved or not needed`)
    );
  ...
  const expectedText = new RegExp(
    `(Successfully swapped .* ${trade.input.symbol} for .* ${trade.output.symbol})`
  );
  await expect(
    page.locator("div", { hasText: expectedText }).last()
...
async function handleToken(
  ...
  const selectorInfix = `${
    type === InputType.INPUT ? "input" : "output"
  }-currency${type === InputType.INPUT ? "0" : "1"}`;
  ...
  await page
    .locator(
      `[testdata-id=swap-${selectorInfix}-token-selector-dialog-row-${token.address}]`
    )
    .click();
