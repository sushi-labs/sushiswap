...
export const CurrencyInput: FC<CurrencyInput> = ({
  ...
  // Usd pct change
  const srcTokenPrice = trade?.inputAmount.currency
    ? prices?.[trade.inputAmount.currency.wrapped.address]
    : undefined;
  const dstTokenPrice = trade?.outputAmount.currency
    ? prices?.[trade.outputAmount.currency.wrapped.address]
    : undefined;
  const usdPctChange = useMemo(() => {
    const inputUSD =
      trade?.inputAmount && srcTokenPrice
        ? trade.inputAmount.multiply(srcTokenPrice.asFraction)
        : undefined;
    const outputUSD =
      trade?.outputAmount && dstTokenPrice
        ? trade.outputAmount.multiply(dstTokenPrice.asFraction)
        : undefined;
  ...
  return (
    <Web3Input.Currency
      ...
      disabled={disabled}
      usdPctChange={
        inputType === TradeType.EXACT_OUTPUT ? usdPctChange : undefined
      }
    />
  );
};
