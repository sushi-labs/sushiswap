...
export const ComplexRoute: FC<{ trade: TradeOutput }> = ({ trade }) => {
  ...
  const finalPaths = trade.route.legs.filter(
    (leg) => leg.tokenTo.address === trade.outputAmount.currency.wrapped.address
  )
  return (
    <>
      <div className="grid items-center grid-flow-col gap-4 grid-col-4">
        ...
        <div className="space-y-2">
          {initialPaths.map((initialPath, i) => (
            <div
              key={i}
              className="z-10 flex items-center gap-4 p-2 text-xs font-medium leading-4 shadow shadow-slate-900 text-slate-300 bg-slate-800 rounded-2xl"
            >
        ...
        <div className="space-y-2">
          {percentPaths.map((percentagePath, i) => (
            <div
              key={i}
              className="z-10 grid items-center grid-flow-col gap-4 p-2 text-xs font-medium leading-4 shadow shadow-slate-900 text-slate-300 bg-slate-800 rounded-2xl"
            >
