      ...
      {trade.route.legs.map((leg, i) => (
        <Tooltip
          ...
          button={
            ...
            >
              <Currency.Icon
                currency={tokenFromRToken(leg.tokenFrom)}
                width={20}
                height={20}
              />
              {legs < 3 ? (
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-center">
                ...
                >
                  ...
                  <span className="text-slate-500">/</span> {leg.tokenTo.symbol}
                </Typography>
                <Link.External
                  href={chains[trade.inputAmount.currency.chainId].getTokenUrl(
                    leg.poolAddress
                  )}
                >
              ...
              <Typography
                variant="xs"
                weight={600}
                className="flex gap-1.5 items-end text-slate-400"
              >
                <Chip color="gray" size="sm" label={leg.poolType} />
                <Chip
                  color="gray"
                  size="sm"
                  label={`Fee ${leg.poolFee * 100}%`}
                />
              </Typography>
            </div>
          }
