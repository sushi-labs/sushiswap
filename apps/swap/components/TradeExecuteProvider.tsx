...
const KLIMA_FEE = Amount.fromRawAmount(
...
export const TradeExecuteProvider: FC<TradeExecuteProvider> = ({
  ...
  const inputCurrencyRebase = useBentoBoxTotal(
    chainId,
    trade?.inputAmount.currency
  );
  const outputCurrencyRebase = useBentoBoxTotal(
    chainId,
    trade?.outputAmount.currency
  );
  const [sushiSwapRouter, tridentRouter, sushiSwapKlimaRouter] =
    useRouters(chainId);
    ...
    async (
      ...
      try {
        ...
        if (trade.isV1()) {
          ...
          const shouldCarbonOffset =
            chainId === ChainId.POLYGON && carbonOffset;

          if (trade.inputAmount.currency.isNative) {
            value = shouldCarbonOffset
              ? trade.inputAmount.add(KLIMA_FEE).toHex()
              : trade.inputAmount.toHex();
          ...
          call = {
            address:
              shouldCarbonOffset && sushiSwapKlimaRouter
                ? sushiSwapKlimaRouter.address
                : sushiSwapRouter.address,
            calldata: sushiSwapRouter.interface.encodeFunctionData(
              methodName,
              args
            ),
            value,
          ...
          if (!tridentRouter || !inputCurrencyRebase || !outputCurrencyRebase)
            return;

          const actions = [
            approveMasterContractAction({ router: tridentRouter, signature }),
          ];

          if (trade.isSinglePool()) {
            actions.push(
              tridentRouter.interface.encodeFunctionData(
            ...
            const [initialPath, percentagePath, output] =
              trade.route.legs.reduce<
                [
                  {
                    tokenIn: string;
                    pool: string;
                              ...
                              ? BigNumber.from(
                                  ...
                                  initialPath.reduce(
                                    (previousValue, currentValue) =>
                                      previousValue.add(currentValue.amount),
                                    Zero
                                  )
                                )
                              : BigNumber.from(
                    ...
                    return [
                      ...
                      [
                        ...percentagePath,
                        {
                          ...
                          pool: leg.poolAddress,
                          balancePercentage: getBigNumber(
                            leg.swapPortion * 10 ** 8
                          ),
                          data: defaultAbiCoder.encode(
                            ["address", "address", "bool"],
                            [
                  ...
                  [
                    {
                      ...
                      to: trade.outputAmount.currency.isNative
                        ? tridentRouter.address
                        : account,
                      unwrapBento: true,
                      minAmount: trade
            ...
            actions.push(
              tridentRouter.interface.encodeFunctionData("complexPath", [
        ...
        if (call) {
          if (!isAddress(call.address))
            new Error("call address has to be an address");
          if (call.address === AddressZero)
            new Error("call address cannot be zero");
          ...
          console.log("About to actually setRequest with...", {
            ...
            ...("gasEstimate" in estimatedCall
              ? { gasLimit: calculateGasMargin(estimatedCall.gasEstimate) }
              : {}),
          });
    ...
    (data: SendTransactionResult | undefined) => {
      ...
      createNotification({
        ...
        summary: {
          ...
          completed: `Successfully swapped ${trade.inputAmount.toSignificant(
            6
          )} ${
            trade.inputAmount.currency.symbol
          } for ${trade.outputAmount.toSignificant(6)} ${
  ...
  const { sendTransaction, isLoading: isWritePending } = useSendTransaction({
    ...
    enabled:
      trade &&
      (trade.route.status === RouteStatus.Success ||
        trade.route.status === RouteStatus.Partial) &&
      approved,
    onSuccess,
  });
