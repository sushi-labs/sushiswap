...
import { useSingleContractMultipleData } from '../../state/multicall'
import { ErrorState, LoadingState, SuccessState, UseTokenBalance, UseTokenBalances } from './types'

export const useTokenBentoboxBalances: UseTokenBalances = (chainId, account, tokens) => {
  const contract = useBentoBoxContract(chainId)
  ...
  const totals = useSingleContractMultipleData(chainId, latestBlockNumber, contract, 'totals', validatedTokenAddresses)
  ...
  const balances = useSingleContractMultipleData(chainId, latestBlockNumber, contract, 'balanceOf', balanceInput)
...
export const useTokenBentoboxBalance: UseTokenBalance = (chainId, account, token) => {
  const tokens = useMemo(() => [token], [token])
  const { isError, isLoading, data } = useTokenBentoboxBalances(chainId, account, tokens)
