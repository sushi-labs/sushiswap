...
const STABLE_POOL_FACTORY_ABI_INTERFACE = new Interface(
  STABLE_POOL_FACTORY_ABI
);

export function useStableProductPoolFactoryContract(
  ...
  return useMemo(() => {
    ...
    return new Contract(
      STABLE_POOL_FACTORY_ADDRESS[chainId],
      STABLE_POOL_FACTORY_ABI_INTERFACE,
      provider
    );
  }, [chainId, provider]);
}
