...
export const VolumeChart: FC<{ stats?: Pair[] }> = ({ stats }) => {
  ...
  const [xData, yData] = useMemo(() => {
    const key =
      chartTimespans[chartPeriod] <= chartTimespans[TvlChartPeriod.Week]
        ? "hourSnapshots"
        : "daySnapshots";
    ...
    stats?.forEach((pair) => {
      const [_x, _y] = pair[key].reduce<[number[], number[]]>(
        (acc, snapshot) => {
          if (
            snapshot.date * 1000 >=
            currentDate - chartTimespans[chartPeriod]
          ) {
  ...
  const onMouseOver = useCallback(
    ({ name, value }: { name: number; value: number }) => {
      ...
      const nameNodes = document.getElementsByClassName(
        "hoveredItemNameVolume"
      );
  ...
  return (
    <div className="flex flex-col gap-3">
        ...
        <div className="flex gap-4">
          <button
            ...
            className={classNames(
              ...
              chartPeriod === TvlChartPeriod.Day
                ? "text-blue"
                : "text-slate-500"
            )}
          >
      ...
      <div className="flex flex-col">
        ...
        <Typography
          variant="sm"
          className="text-slate-500 hoveredItemNameVolume"
        >
          {format(
