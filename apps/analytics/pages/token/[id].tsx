...
export const getServerSideProps: GetServerSideProps = async ({
  ...
  const [token, bundles] = await Promise.all([
    getToken(query.id as string),
    getBundles(),
  ]);
  return {
    props: {
...
const TokenPage: FC<InferGetServerSidePropsType<typeof getServerSideProps>> = ({
  fallback,
}) => {
  return (
...
const _TokenPage: FC = () => {
  ...
  const { data } = useSWR<{ token: GraphToken }>(
    `/analytics/api/token/${router.query.id}`,
