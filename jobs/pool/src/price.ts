...
import { fetchBlockNumber, readContracts } from '@wagmi/core'
...
async function transform(chainId: ChainId, pools: Pool[]) {
  ...
  const blockNumber = await fetchBlockNumber(config, { chainId })
  ...
  pools.forEach((pool) => {
    ...
    if (
      ...
      const tickSpacing =
        TICK_SPACINGS[(pool.swapFee * 1_000_000) as keyof typeof TICK_SPACINGS]
      if (v3 && tickSpacing) {
        rPools.push(
          new CLRPool(
            pool.address as Address,
