...
const PROTOCOL = ProtocolName.SPIRITSWAP
...
export async function spiritSwapV2() {
...
async function start(client: PrismaClient) {
  console.log(
    `Fetching pools from ${PROTOCOL} ${VERSION}, chains: ${SPIRITSWAP_V2_SUPPORTED_CHAINS.map(
  ...
  for (const chainId of SPIRITSWAP_V2_SUPPORTED_CHAINS) {
    ...
    if (!SPIRITSWAP_V2_SUBGRAPH_NAME[chainId]) {
      console.log(`Subgraph not found: ${chainId} ${SPIRITSWAP_V2_SUBGRAPH_NAME[chainId]}, Skipping`)
      continue
    }
    console.log(`Loading data from chain: ${chainName[chainId]}(${chainId}), ${SPIRITSWAP_V2_SUBGRAPH_NAME[chainId]}`)
    let pairCount = 0
    let cursor = ''
    ...
    console.log(
      `Finished loading pairs from ${GRAPH_HOST[chainId]}/${SPIRITSWAP_V2_SUBGRAPH_NAME[chainId]}, ${pairCount} pairs`
    )
  }
