...
export async function spiritSwapV2({ dryRun, initialRun }: { dryRun: boolean; initialRun: boolean }) {
  if (initialRun) {
    console.log('SPIRITSWAP V2: Initial run')
  }
  if (dryRun) {
    console.log('SPIRITSWAP V2: Dry run')
  }
  const client = await createClient()
  const configs = SPIRITSWAP_V2_SUPPORTED_CHAINS.map((chainId) => {
    return {
      chainId,
      subgraph: SPIRITSWAP_V2_SUBGRAPH_NAME[chainId],
      graphHost: GRAPH_HOST[chainId],
      protocol: ProtocolName.SPIRITSWAP,
