...
import { getBuiltGraphSDK, V2PairsQuery } from '../../../../.graphclient/index.js.js.js.js.js'
...
const PROTOCOL = ProtocolName.NETSWAP
...
async function start(client: PrismaClient) {
  ...
  for (const chainId of NETSWAP_V2_SUPPORTED_CHAINS) {
    ...
    if (!NETSWAP_V2_SUBGRAPH_NAME[chainId]) {
      console.log(`Subgraph not found: ${chainId} ${NETSWAP_V2_SUBGRAPH_NAME[chainId]}, Skipping`)
      continue
    }
    console.log(`Loading data from chain: ${chainName[chainId]}(${chainId}), ${NETSWAP_V2_SUBGRAPH_NAME[chainId]}`)
    let pairCount = 0
    let cursor = ''
    ...
    console.log(
      `Finished loading pairs from ${GRAPH_HOST[chainId]}/${NETSWAP_V2_SUBGRAPH_NAME[chainId]}, ${pairCount} pairs`
    )
  }
