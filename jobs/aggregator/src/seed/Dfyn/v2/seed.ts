...
export async function dfynV2({ dryRun, initialRun }: { dryRun: boolean; initialRun: boolean }) {
  if (initialRun) {
    console.log('DFYN V2: Initial run')
  }
  if (dryRun) {
    console.log('DFYN V2: Dry run')
  }
  const client = await createClient()
  const configs = DFYN_V2_SUPPORTED_CHAINS.map((chainId) => {
    return {
      chainId,
      subgraph: DFYN_V2_SUBGRAPH_NAME[chainId],
      graphHost: GRAPH_HOST[chainId],
      protocol: ProtocolName.DFYN,
