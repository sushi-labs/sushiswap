...
export async function elkV2({ dryRun, initialRun }: { dryRun: boolean; initialRun: boolean }) {
  if (initialRun) {
    console.log('ELK V2: Initial run')
  }
  if (dryRun) {
    console.log('ELK V2: Dry run')
  }
  const client = await createClient()
  const configs = ELK_V2_SUPPORTED_CHAINS.map((chainId) => {
    return {
      chainId,
      subgraph: ELK_V2_SUBGRAPH_NAME[chainId],
      graphHost: GRAPH_HOST[chainId],
      protocol: ProtocolName.ELK,
