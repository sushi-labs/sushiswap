...
export class UniswapSchema {
  ...
  static async getPools(sdk: Sdk, query: V2PairsQueryVariables) {
    const result = await sdk.V2Pairs(query)
    const sorted = result.V2_pairs.sort((a, b) => Number(b.createdAtTimestamp) - Number(a.createdAtTimestamp))
  ...
  static transform(
    ...
    if (!type || !swapFee || !twapEnabled) {
      throw new Error(
        `${this.logPrefix(
          config
        )} - ERROR: Pool type, swap fee and twapEnabled are all required when transforming pools using UNISWAP schema.`
      )
    }
