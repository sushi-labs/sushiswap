...
import { MessariSchema } from '../../base/messari.js'
...
export async function apeSwapV2({ dryRun, initialRun }: { dryRun: boolean; initialRun: boolean }) {
  if (initialRun) {
    console.log('APESWAP V2: Initial run')
  }
  if (dryRun) {
    console.log('APESWAP V2: Dry run')
  }
  const client = await createClient()
  const configs = APESWAP_V2_SUPPORTED_CHAINS.map((chainId) => {
    return {
      chainId,
      subgraph: APESWAP_V2_SUBGRAPH_NAME[chainId],
      graphHost: GRAPH_HOST[chainId],
      protocol: ProtocolName.APESWAP,
  ...
  for (const config of configs) {
    const newestPool = await MessariSchema.run({ client, config, initialRun, dryRun })
