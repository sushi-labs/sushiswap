...
export class PancakeSwapSchema {
      ...
      if (initialRun) {
        ...
        do {
          ...
          if (!dryRun) {
            ...
            console.log(
              `${this.logPrefix(config)} - ${chainName[chainId]}(${chainId}), ${graphHost}/${subgraph}` +
                `: This IS A DRY RUN, ${pairCount} pools would have been created.`
            )
          }
        } while (cursor !== '')
        console.log(
        ...
        const query = newestPool
          ? ({
              first: 1000,
              where: {
                timestamp_gt: newestPool.timestamp,
              },
              orderBy: 'timestamp',
              orderDirection: 'desc',
            } as PCSPairsQueryVariables)
          : ({
              first: 1000,
              orderBy: 'timestamp',
              orderDirection: 'desc',
        ...
        if (!result.MINIMAL_pairs.length) {
  ...
  static async getPools(sdk: Sdk, query: PCSPairsQueryVariables) {
    const result = await sdk.PCSPairs(query)
    const sorted = result.MINIMAL_pairs.sort((a, b) => Number(b.timestamp) - Number(a.timestamp))
    const newestPool = result.MINIMAL_pairs.length
      ? ({ address: sorted[0].id, timestamp: Number(sorted[0].timestamp) } as NewestPool)
      : undefined
  ...
  static transform(
    ...
    if (!type || !swapFee || !twapEnabled) {
      throw new Error(
        `${this.logPrefix(
          config
        )} - ERROR: Pool type, swap fee and twapEnabled are all required when transforming pools using PCS Schema.`
      )
    }
