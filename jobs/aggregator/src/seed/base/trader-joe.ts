...
export class TraderJoeSchema {
    ...
    try {
      if (initialRun) {
        ...
        do {
          ...
          const currentPairCount = result.TJ_pairs.length
          if (currentPairCount === batchSize) {
            cursor = result.TJ_pairs[currentPairCount - 1]?.id ?? ''
          }
        ...
        const query = newestPool
          ? ({
              ...
              orderBy: 'timestamp',
              orderDirection: 'desc',
            } as TraderJoePairsQueryVariables)
          : ({
              first: 1000,
              orderBy: 'timestamp',
              orderDirection: 'desc',
  ...
  static async getPools(sdk: Sdk, query: TraderJoePairsQueryVariables) {
    const result = await sdk.TraderJoePairs(query)

    // TODO: fix this. timestamp, update query?
    const sorted = result.TJ_pairs.sort((a, b) => Number(b.timestamp) - Number(a.timestamp))
  ...
  static transform(
    ...
    if (!type || !swapFee || !twapEnabled) {
      throw new Error(
        `${this.logPrefix(
          config
        )} - ERROR: Pool type, swap fee and twapEnabled are all required when transforming pools using Trader Joe schema.`
      )
    }
