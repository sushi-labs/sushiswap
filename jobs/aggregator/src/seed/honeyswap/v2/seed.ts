...
export async function honeySwapV2({ dryRun, initialRun }: { dryRun: boolean; initialRun: boolean }) {
  if (initialRun) {
    console.log('HONEYSWAP V2: Initial run')
  }
  if (dryRun) {
    console.log('HONEYSWAP V2: Dry run')
  }
  const client = await createClient()
  const configs = HONEYSWAP_V2_SUPPORTED_CHAINS.map((chainId) => {
    return {
      chainId,
      subgraph: HONEYSWAP_V2_SUBGRAPH_NAME[chainId],
