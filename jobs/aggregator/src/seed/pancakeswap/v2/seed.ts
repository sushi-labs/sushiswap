...
import { PancakeSwapSchema } from '../../base/pcs.js'
import { GRAPH_HOST, PANCAKESWAP_V2_SUBGRAPH_NAME, PANCAKESWAP_V2_SUPPORTED_CHAINS } from '../config.js'

const LAST_CREATED_POOL_TIMESTAMP_BY_CHAIN_ID: Map<ChainId, NewestPool> = new Map()

export async function pancakeSwapV2({ dryRun, initialRun }: { dryRun: boolean; initialRun: boolean }) {
  if (initialRun) {
    console.log('PANCAKESWAP V2: Initial run')
  }
  if (dryRun) {
    console.log('PANCAKESWAP V2: Dry run')
  }
  const client = await createClient()
  const configs = PANCAKESWAP_V2_SUPPORTED_CHAINS.map((chainId) => {
    return {
      chainId,
      subgraph: PANCAKESWAP_V2_SUBGRAPH_NAME[chainId],
      graphHost: GRAPH_HOST[chainId],
      protocol: ProtocolName.PANCAKESWAP,
  ...
  for (const config of configs) {
    const newestPool = await PancakeSwapSchema.run({ client, config, initialRun, dryRun })
