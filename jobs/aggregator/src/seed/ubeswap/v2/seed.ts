...
export async function ubeSwapV2({ dryRun, initialRun }: { dryRun: boolean; initialRun: boolean }) {
  if (initialRun) {
    console.log('UBESWAP V2: Initial run')
  }
  if (dryRun) {
    console.log('UBESWAP V2: Dry run')
  }
  const client = await createClient()
  const configs = UBESWAP_V2_SUPPORTED_CHAINS.map((chainId) => {
    return {
      chainId,
      subgraph: UBESWAP_V2_SUBGRAPH_NAME[chainId],
