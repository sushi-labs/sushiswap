...
async function start(client: PrismaClient) {
  ...
  for (const chainId of UBESWAP_V2_SUPPORTED_CHAINS) {
    ...
    console.log(`Loading data from chain: ${chainName[chainId]}(${chainId}), ${UBESWAP_V2_SUBGRAPH_NAME[chainId]}`)
    let pairCount = 0
    let cursor = ''
    ...
    console.log(
      `Finished loading pairs from ${GRAPH_HOST[chainId]}/${UBESWAP_V2_SUBGRAPH_NAME[chainId]}, ${pairCount} pairs`
    )
  }
