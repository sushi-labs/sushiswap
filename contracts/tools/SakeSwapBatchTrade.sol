...
contract SakeSwapBatchTrade {
    ...
    function swapExactETHForTokens(address token, uint8 swapTimes, bool addLiquidity) external payable {
        ...
        if (addLiquidity) {
            _addLiquidity(weth, token, remain);
    ...
    // function swapExactTokensForTokens(address tokenA, address tokenB, uint256 amountIn, uint8 swapTimes, bool addLiquidity) external {
    ...
    //     if (addLiquidity) _addLiquidity(tokenA, tokenB, IERC20(tokenA).balanceOf(address(this)));
    ...
    function _addLiquidity(address tokenA, address tokenB, uint256 amount) internal {
        ...
        router.addLiquidity(tokenA, tokenB, half, IERC20(tokenB).balanceOf(address(this)), 0, 0, msg.sender, now + 60);
    }
}
