...
async function poolsWithIncentives() {
  ...
  poolsWithIncentives.forEach((result) => {
    console.log(
      ` ${result.id} ${result.name} ${result.isIncentivized} - TOTAL APR: ${((result.totalApr ?? 0) * 100).toFixed(2)}%`
    )
  })
...
async function poolIncentiveStatus() {
  const poolsWithActiveIncentivesCount = await prisma.pool.count({
    where: {
      isIncentivized: true,
      incentives: {
        some: {},
  ...
  const poolsWithInactiveIncentivesCount = await prisma.pool.count({
    where: {
  ...
  console.log('active: ', poolsWithActiveIncentivesCount, 'ran out: ', poolsWithInactiveIncentivesCount)
}
