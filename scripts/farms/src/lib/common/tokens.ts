...
export async function getTokenBalancesOf(
  ...
  const balanceOfCalls: ReadContractsConfig["contracts"] = tokens.map(
    (token) => ({
      address: token,
      args: [address],
      chainId: chainId,
  ...
  const decimalCalls: ReadContractsConfig["contracts"] = tokens.map(
    (token) => ({
      address: token,
      chainId: chainId,
      abi: erc20ABI,
  ...
  const balancesOf = result.splice(
    0,
    balanceOfCalls.length
  ) as unknown as Awaited<ReturnType<ERC20["balanceOf"]>>[];
