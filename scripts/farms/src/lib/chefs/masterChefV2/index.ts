...
export async function getMasterChefV2(): Promise<{
  ...
  const sushiPerDay =
    divBigNumberToNumber(sushiPerBlock, SUSHI[ChainId.ETHEREUM].decimals) *
    blocksPerDay;
  const sushiPriceUSD =
    tokens.find(
      (tokenPrice) =>
  ...
  return {
    ...
    farms: pools.reduce<Record<string, Farm>>((acc, pool) => {
      ...
      const sushiRewardPerDay =
        sushiPerDay *
        (pool.poolInfo.allocPoint.toNumber() / totalAllocPoint.toNumber());
        ...
        if (token) {
          ...
          switch (pool.rewarder.id.toLowerCase()) {
            ...
            case "0x7519c93fc5073e15d89131fd38118d73a72370f8": {
              rewardPerSecond = divBigNumberToNumber(
                pool.rewarder.rewardPerSecond.div(12),
                token.decimals
              );
              break;
            }
