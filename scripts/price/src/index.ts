...
async function getSushiSwapResults() {
  ...
  return results
    .filter(
      (result): result is NonNullable<typeof results[0]> => result !== undefined
    )
    .map((result, i) => {
...
export async function execute() {
  ...
  const results = (
    await Promise.all([getSushiSwapResults(), getTridentResults()])
  ).flat();
  ...
  const combined = chainIds.map((chainId) => {
    ...
    if (sources.length === 1) {
      ...
      sources[0].tokens.forEach((token) =>
        uniqueTokens.set(token.id, token.priceUSD)
      );
  ...
  await redis.hset(
    ...
    Object.fromEntries(
      combined.map(
        ({ chainId, tokens, updatedAtBlock, updatedAtTimestamp }) => [
          chainId,
          stringify({
            chainId,
            ...tokens.reduce(
              (acc, token) => ({ ...acc, [token.id]: token.priceUSD }),
              {}
            ),
            updatedAtBlock,
            updatedAtTimestamp,
